...
BLUEPRINT_NAME = "BP_Cubeless_ED_PCGProfileMatrixSelector"
...
DEFAULT_GRAPH = (
    ...
    "PCG_Cubeless_ED_ProfileMatrix_Both_GroundNormal_DitchNormal."
    "PCG_Cubeless_ED_ProfileMatrix_Both_GroundNormal_DitchNormal"
)
...
print("MCP_CUBELESS_ED_PROFILE_MATRIX_SELECTOR_ASSET_SETUP_BEGIN")
...
print("MCP_CUBELESS_ED_PROFILE_MATRIX_SELECTOR_ASSET_SETUP_END")
"""
    execute_unreal(code)
...
def add_components():
    for component_type, component_name in [
        ...
        ("PCGComponent", "PCG_ProfileMatrix"),
    ]:
        require_success(
...
print("MCP_CUBELESS_ED_PROFILE_MATRIX_SELECTOR_TEMPLATE_SETUP_BEGIN")
...
if not default_graph:
    raise RuntimeError(f"Missing default profile matrix graph: {{DEFAULT_GRAPH}}")
...
for handle in subsystem.k2_gather_subobject_data_for_blueprint(blueprint):
    ...
    if isinstance(obj, unreal.SplineComponent):
        obj.modify()
        obj.set_editor_property("component_tags", ["CubelessEDProfileMatrixSpline"])
    if isinstance(obj, unreal.PCGComponent):
        ...
        obj.set_editor_property("component_tags", ["CubelessEDProfileMatrixSelector"])
        configured.append((obj.get_name(), default_graph.get_path_name()))

unreal.BlueprintEditorLibrary.compile_blueprint(blueprint)
unreal.EditorAssetLibrary.save_loaded_asset(blueprint)
print(f"configured_pcg_templates={{configured}}")
print("MCP_CUBELESS_ED_PROFILE_MATRIX_SELECTOR_TEMPLATE_SETUP_END")
"""
    execute_unreal(code)
...
def add_selector_variable(name, default_value, friendly_name, tooltip):
    require_success(
        "add_blueprint_variable",
        {
            ...
            "is_exposed": True,
            "category": "Cubeless PCG Profile Matrix",
...
def add_selector_variables():
    add_selector_variable(
        ...
        3,
        "Profile Mode",
        "1 = Ground only, 2 = Ditch only, 3 = Both. Apply from the Cubeless ED PCG menu after changing.",
    )
    add_selector_variable(
        "GroundAmountType",
        2,
...
if not blueprint:
    raise RuntimeError(f"Missing profile matrix selector Blueprint: {{BLUEPRINT_PATH}}")
for variable_name in ("ProfileMode", "GroundAmountType", "DitchAmountType"):
...
def main():
    print("MCP_CUBELESS_ED_PROFILE_MATRIX_SELECTOR_BUILD_BEGIN")
    ...
    if not result.get("validation_pass"):
        raise RuntimeError(f"Profile matrix selector Blueprint compile validation failed: {result}")
    ...
    print("MCP_CUBELESS_ED_PROFILE_MATRIX_SELECTOR_BUILD_END")
