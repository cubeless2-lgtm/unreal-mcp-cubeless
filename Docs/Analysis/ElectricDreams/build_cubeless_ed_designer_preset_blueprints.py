...
ACTOR_LABEL_PREFIX = "MCP_Cubeless_ED_PresetBlueprint"
TARGET_SAMPLE_COUNT = 6
TARGET_POINT_SPACING = 320.0

PRESET_BLUEPRINT_SPECS = [
    {
        "name": "GroundOnly",
        "asset_name": "BP_Cubeless_ED_PCGDesigner_GroundOnly",
        "graph_path": (
            "/Game/Cubeless/PCG/ElectricDreamsLearning/Presets/"
            "PCG_Cubeless_ED_Preset_GroundOnly.PCG_Cubeless_ED_Preset_GroundOnly"
        ),
        "expected_points": 8,
    ...
    {
        "name": "DitchOnly",
        "asset_name": "BP_Cubeless_ED_PCGDesigner_DitchOnly",
        "graph_path": (
            "/Game/Cubeless/PCG/ElectricDreamsLearning/Presets/"
            "PCG_Cubeless_ED_Preset_DitchOnly.PCG_Cubeless_ED_Preset_DitchOnly"
        ),
        "expected_points": 42,
    ...
    {
        "name": "Balanced",
        "asset_name": "BP_Cubeless_ED_PCGDesigner_Balanced",
        "graph_path": (
            "/Game/Cubeless/PCG/ElectricDreamsLearning/Presets/"
            "PCG_Cubeless_ED_Preset_Balanced.PCG_Cubeless_ED_Preset_Balanced"
        ),
        "expected_points": 50,
...
def spawn_validation_actor(spec, graph, index):
    ...
    if not actor_class:
        raise RuntimeError(f"Missing preset Blueprint class: {blueprint_class_path(spec)}")

    actor = unreal.EditorLevelLibrary.spawn_actor_from_class(
        actor_class,
        unreal.Vector(7600 + index * 420, 0, 0),
...
def main():
    print("MCP_CUBELESS_ED_PRESET_BLUEPRINTS_BUILD_BEGIN")
    unreal.get_editor_subsystem(unreal.LevelEditorSubsystem).load_level(LEVEL)
    for index, spec in enumerate(PRESET_BLUEPRINT_SPECS):
        graph = unreal.EditorAssetLibrary.load_asset(spec["graph_path"])
        if not graph:
            raise RuntimeError(f"Missing preset graph for {spec['name']}: {spec['graph_path']}")
    ...
    print("MCP_CUBELESS_ED_PRESET_BLUEPRINTS_BUILD_END")
