...
SCRIPT_DIR = pathlib.Path(
    globals().get(
        "__file__",
        r"D:\Git\unreal-mcp-cubeless\Docs\Analysis\ElectricDreams\apply_cubeless_ed_profile_matrix_selector.py",
    )
).parent
BUILDER_SCRIPT = SCRIPT_DIR / "build_cubeless_ed_profile_matrix_presets.py"
...
def load_profile_matrix_config():
    ...
    exec(code, namespace)
    graph_paths = {}
    for spec in namespace["PROFILE_MATRIX_SPECS"]:
        key = (spec["profile_mode"], spec["ground_amount_type"], spec["ditch_amount_type"])
        graph_paths[key] = namespace["PROFILE_MATRIX_GRAPH_PATHS"][spec["name"]]
    return graph_paths


PROFILE_MATRIX_GRAPH_BY_AXES = load_profile_matrix_config()
...
def get_profile_matrix_axes(actor):
    ...
    if profile_mode == 1:
        ...
        ground_type = 0
    key = (profile_mode, ground_type, ditch_type)
    if key not in PROFILE_MATRIX_GRAPH_BY_AXES:
        key = (3, 2, 2)
    return key
...
def apply_profile_matrix_selector(actor, force=True):
    profile_mode, ground_type, ditch_type = get_profile_matrix_axes(actor)
    graph_path = PROFILE_MATRIX_GRAPH_BY_AXES[(profile_mode, ground_type, ditch_type)]
    ...
    if not components:
        raise RuntimeError(f"Profile matrix selector actor has no PCG component: {actor.get_actor_label()}")
...
def is_profile_matrix_selector_actor(actor):
...
def main():
    print("MCP_CUBELESS_ED_PROFILE_MATRIX_SELECTOR_APPLY_BEGIN")
    selected = [actor for actor in get_selected_level_actors() if is_profile_matrix_selector_actor(actor)]
    actors = selected or [actor for actor in get_all_level_actors() if is_profile_matrix_selector_actor(actor)]
    if not actors:
        raise RuntimeError("No Cubeless ED profile matrix selector actors found")
    print(f"profile_matrix_selector_actor_count={len(actors)}")
    for actor in actors:
        result = apply_profile_matrix_selector(actor, force=True)
    ...
    print("MCP_CUBELESS_ED_PROFILE_MATRIX_SELECTOR_APPLY_END")
