...
SCRIPT_DIR = pathlib.Path(
    globals().get(
        "__file__",
        pathlib.Path.cwd() / "Docs" / "Analysis" / "ElectricDreams" / "verify_cubeless_ed_designer_preset_blueprints.py",
    )
).resolve().parent

BUILDER_SCRIPT = str(SCRIPT_DIR / "build_cubeless_ed_designer_preset_blueprints.py")
VERIFY_MARKER = "MCP_CUBELESS_ED_PRESET_BLUEPRINTS_VERIFY_BEGIN"


def load_builder_config():
    namespace = {"__name__": "_cubeless_ed_designer_preset_blueprints_config"}
    ...
    for idx, entry in enumerate(entries):
        ...
        rows.append({
            ...
            "designer_control_layer_pass": bool(
                helpers.get_bool_attribute_by_metadata_key(entry, metadata, "DesignerControlLayerPass")
            ),
            "designer_preset_id": int(
                helpers.get_integer32_attribute_by_metadata_key(entry, metadata, "DesignerPresetId")
            ),
            "designer_preset_type": int(
                helpers.get_integer32_attribute_by_metadata_key(entry, metadata, "DesignerPresetType")
            ),
            "designer_preset_pass": bool(
                helpers.get_bool_attribute_by_metadata_key(entry, metadata, "DesignerPresetPass")
            ),
        })
    return rows
...
def validate_blueprint(spec, config):
    ...
    if not blueprint:
        raise RuntimeError(f"Missing preset Blueprint: {config['blueprint_path'](spec)}")
    ...
    profile_name_by_id = {10: "GroundControls", 20: "DitchHierarchy"}
    profile_type_by_id = {10: 1, 20: 2}
    for row in rows:
    ...
    preset_id_mismatch_count = sum(1 for row in rows if row["designer_preset_id"] != spec["preset_id"])
    preset_type_mismatch_count = sum(1 for row in rows if row["designer_preset_type"] != spec["preset_type"])
...
def main():
    ...
    results = [validate_blueprint(spec, config) for spec in config["PRESET_BLUEPRINT_SPECS"]]
    log_path, marker_found, log_errors = scan_latest_log("MCP_CUBELESS_ED_PRESET_BLUEPRINTS_BUILD_BEGIN")
    ...
    print(f"preset_blueprints_validation_pass={validation_pass}")
    print("MCP_CUBELESS_ED_PRESET_BLUEPRINTS_VERIFY_END")
    if not validation_pass:
        raise RuntimeError("Cubeless ED preset Blueprint verification failed")
