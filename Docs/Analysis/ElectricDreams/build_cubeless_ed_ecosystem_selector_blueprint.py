...
BLUEPRINT_NAME = "BP_Cubeless_ED_PCGEcosystemSelector"
...
print("MCP_CUBELESS_ED_ECOSYSTEM_SELECTOR_ASSET_SETUP_BEGIN")
...
print("MCP_CUBELESS_ED_ECOSYSTEM_SELECTOR_ASSET_SETUP_END")
"""
    execute_unreal(code)
...
def add_components():
    for component_type, component_name in [
        ...
        ("PCGComponent", "PCG_MaterialOverride"),
    ]:
        require_success(
...
print("MCP_CUBELESS_ED_ECOSYSTEM_SELECTOR_TEMPLATE_SETUP_BEGIN")
...
for handle in subsystem.k2_gather_subobject_data_for_blueprint(blueprint):
    ...
    if isinstance(obj, unreal.SplineComponent):
        obj.modify()
        obj.set_editor_property("component_tags", ["CubelessEDEcosystemSpline"])
    if isinstance(obj, unreal.PCGComponent):
        obj.modify()
        if obj.get_name().startswith("PCG_TreeProfile"):
            graph = default_tree_graph
        elif obj.get_name().startswith("PCG_MaterialOverride"):
        ...
        obj.set_editor_property("component_tags", ["CubelessEDEcosystemSelector"])
        configured.append((obj.get_name(), graph.get_path_name()))

unreal.BlueprintEditorLibrary.compile_blueprint(blueprint)
unreal.EditorAssetLibrary.save_loaded_asset(blueprint)
print(f"configured_pcg_templates={{configured}}")
print("MCP_CUBELESS_ED_ECOSYSTEM_SELECTOR_TEMPLATE_SETUP_END")
"""
    execute_unreal(code)
...
def add_selector_variables():
    add_selector_variable(
        ...
        3,
        "Ecosystem Mode",
        "1 = Style only, 2 = Tree only, 3 = Combined.",
    )
    add_selector_variable(
        "VisualStyleType",
        1,
        "Visual Style Type",
        "1 = ClassicGrass, 2 = TallGrass, 3 = MixedGrass, 4 = GroundFoliage, 5 = SmallRocks.",
        5,
    )
    add_selector_variable(
        "ProfileMode",
    ...
    add_selector_variable(
        ...
        1,
        "Tree Style Type",
        "1 = CompactConifer, 2 = ColumnConifer, 3 = MixedConifer.",
    )
    add_selector_variable(
        "TreeAmountType",
        2,
        "Tree Amount Type",
        "1 = Solo, 2 = Sparse, 3 = LightGrove.",
    )
    add_selector_variable(
        "MaterialDomainType",
        1,
        "Material Domain Type",
        "1 = GroundFoliage, 2 = SmallRocks, 3 = CompactConifer. Material preview is generated alongside the active ecosystem mode.",
    )
    add_selector_variable(
        "MaterialVariantType",
        2,
...
def main():
    print("MCP_CUBELESS_ED_ECOSYSTEM_SELECTOR_BUILD_BEGIN")
    ...
    if not result.get("validation_pass"):
        raise RuntimeError(f"Ecosystem selector Blueprint compile validation failed: {result}")
    ...
    print("MCP_CUBELESS_ED_ECOSYSTEM_SELECTOR_BUILD_END")
