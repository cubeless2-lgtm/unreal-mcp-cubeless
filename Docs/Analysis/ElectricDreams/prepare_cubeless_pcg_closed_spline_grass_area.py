...
def main():
    if not SCRIPT_PATH.exists():
        raise RuntimeError(f"Missing Cubeless closed spline grass script: {SCRIPT_PATH}")
    namespace = {"__name__": "__main__", "__file__": str(SCRIPT_PATH)}
    with SCRIPT_PATH.open("r", encoding="utf-8") as handle:
        exec(compile(handle.read(), str(SCRIPT_PATH), "exec"), namespace)
    print("pcg_closed_spline_grass_area_scheduled=True")
    print(
        "pcg_closed_spline_grass_area_note="
