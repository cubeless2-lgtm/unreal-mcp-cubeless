...
BLUEPRINT_NAME = "BP_Cubeless_ED_PCGStyleProfileMatrixSelector"
...
DEFAULT_GRAPH = (
    ...
    "PCG_Cubeless_ED_StyleProfileMatrix_ClassicGrass_Both_GroundNormal_DitchNormal."
    "PCG_Cubeless_ED_StyleProfileMatrix_ClassicGrass_Both_GroundNormal_DitchNormal"
)
...
print("MCP_CUBELESS_ED_STYLE_PROFILE_MATRIX_SELECTOR_ASSET_SETUP_BEGIN")
...
print("MCP_CUBELESS_ED_STYLE_PROFILE_MATRIX_SELECTOR_ASSET_SETUP_END")
"""
    execute_unreal(code)
...
def add_components():
    for component_type, component_name in [
        ...
        ("PCGComponent", "PCG_StyleProfileMatrix"),
    ]:
        require_success(
...
print("MCP_CUBELESS_ED_STYLE_PROFILE_MATRIX_SELECTOR_TEMPLATE_SETUP_BEGIN")
...
if not default_graph:
    raise RuntimeError(f"Missing default style profile matrix graph: {{DEFAULT_GRAPH}}")
...
for handle in subsystem.k2_gather_subobject_data_for_blueprint(blueprint):
    ...
    if isinstance(obj, unreal.SplineComponent):
        obj.modify()
        obj.set_editor_property("component_tags", ["CubelessEDStyleProfileMatrixSpline"])
    if isinstance(obj, unreal.PCGComponent):
        ...
        obj.set_editor_property("component_tags", ["CubelessEDStyleProfileMatrixSelector"])
        configured.append((obj.get_name(), default_graph.get_path_name()))

unreal.BlueprintEditorLibrary.compile_blueprint(blueprint)
unreal.EditorAssetLibrary.save_loaded_asset(blueprint)
print(f"configured_pcg_templates={{configured}}")
print("MCP_CUBELESS_ED_STYLE_PROFILE_MATRIX_SELECTOR_TEMPLATE_SETUP_END")
"""
    execute_unreal(code)


def add_selector_variable(name, default_value, friendly_name, tooltip, clamp_max=3):
    require_success(
        "add_blueprint_variable",
        {
            ...
            "is_exposed": True,
            "category": "Cubeless PCG Style Profile Matrix",
...
def add_selector_variables():
    add_selector_variable(
        ...
        "Visual Style Type",
        (
            "1 = ClassicGrass, 2 = TallGrass, 3 = MixedGrass, "
            "4 = GroundFoliage, 5 = SmallRocks. Apply from the Cubeless ED PCG menu after changing."
        ),
        5,
    )
    add_selector_variable(
        ...
        3,
        "Profile Mode",
        "1 = Ground only, 2 = Ditch only, 3 = Both.",
    )
    add_selector_variable(
        "GroundAmountType",
        2,
        "Ground Amount Type",
        "1 = Sparse, 2 = Normal, 3 = Dense. Ignored when Profile Mode is Ditch only.",
    )
    add_selector_variable(
        "DitchAmountType",
        2,
        "Ditch Amount Type",
        "1 = Sparse, 2 = Normal, 3 = Dense. Ignored when Profile Mode is Ground only.",
    )
    code = f"""
import unreal
...
if not blueprint:
    raise RuntimeError(f"Missing style profile matrix selector Blueprint: {{BLUEPRINT_PATH}}")
for variable_name in ("VisualStyleType", "ProfileMode", "GroundAmountType", "DitchAmountType"):
...
def main():
    print("MCP_CUBELESS_ED_STYLE_PROFILE_MATRIX_SELECTOR_BUILD_BEGIN")
    ...
    if not result.get("validation_pass"):
        raise RuntimeError(f"Style profile matrix selector Blueprint compile validation failed: {result}")
    ...
    print("MCP_CUBELESS_ED_STYLE_PROFILE_MATRIX_SELECTOR_BUILD_END")
