...
SCRIPT_DIR = pathlib.Path(
    globals().get(
        "__file__",
        pathlib.Path.cwd() / "Docs" / "Analysis" / "ElectricDreams" / "verify_cubeless_ed_ground_amount_blueprints.py",
    )
).resolve().parent

BUILDER_SCRIPT = str(SCRIPT_DIR / "build_cubeless_ed_ground_amount_blueprints.py")
VERIFY_MARKER = "MCP_CUBELESS_ED_GROUND_AMOUNT_BLUEPRINTS_VERIFY_BEGIN"


def load_builder_config():
    namespace = {"__name__": "_cubeless_ed_ground_amount_blueprints_config"}
...
def validate_blueprint(spec, config):
    ...
    profile_type_mismatch_count = sum(
        1 for row in rows
        if row["designer_profile_id"] != 10 or row["designer_profile_type"] != 1
    )
    ...
    validation_pass = all([
        ...
        dict(profile_counts) == {10: spec["expected_points"]},
...
def main():
    ...
    log_path, marker_found, log_errors = scan_latest_log("MCP_CUBELESS_ED_GROUND_AMOUNT_BLUEPRINTS_BUILD_BEGIN")
    ...
    print(f"ground_amount_blueprints_validation_pass={validation_pass}")
    print("MCP_CUBELESS_ED_GROUND_AMOUNT_BLUEPRINTS_VERIFY_END")
    if not validation_pass:
        raise RuntimeError("Cubeless ED ground amount Blueprint verification failed")
