...
PACKAGE_PATH = "/Game/Cubeless/PCG/ElectricDreamsLearning/DesignerProfileMatrixCombos"
...
ACTOR_LABEL_PREFIX = "MCP_Cubeless_ED_ProfileMatrix"
TARGET_SAMPLE_COUNT = 6
TARGET_POINT_SPACING = 320.0

AMOUNT_PRESET_PATH = "/Game/Cubeless/PCG/ElectricDreamsLearning/AmountPresets"
MATRIX_PRESET_PATH = "/Game/Cubeless/PCG/ElectricDreamsLearning/DesignerMatrixCombos"
...
def amount_graph_path(asset_name):
    return f"{AMOUNT_PRESET_PATH}/{asset_name}.{asset_name}"


def matrix_graph_path(ground, ditch):
    asset_name = f"PCG_Cubeless_ED_Matrix_{ground['name']}_{ditch['name']}"
    return f"{MATRIX_PRESET_PATH}/{asset_name}.{asset_name}"
...
def make_ground_only_spec(ground):
    ...
    return {
        "name": f"GroundOnly_{ground['name']}",
        "asset_name": f"PCG_Cubeless_ED_ProfileMatrix_GroundOnly_{ground['name']}",
        ...
        "subgraph_path": amount_graph_path(ground["graph_asset"]),
...
def make_ditch_only_spec(ditch):
    ground_type = 0
    ditch_type = ditch["amount_type"]
    profile_mode = PROFILE_MODE_DITCH_ONLY
    matrix_type = profile_matrix_type(profile_mode, ground_type, ditch_type)
    return {
        "name": f"DitchOnly_{ditch['name']}",
        "asset_name": f"PCG_Cubeless_ED_ProfileMatrix_DitchOnly_{ditch['name']}",
        "profile_mode": profile_mode,
        "ground_amount_type": ground_type,
        "ditch_amount_type": ditch_type,
        "profile_matrix_type": matrix_type,
        "profile_matrix_id": profile_matrix_id(profile_mode, ground_type, ditch_type),
        "subgraph_path": amount_graph_path(ditch["graph_asset"]),
...
def make_both_spec(ground, ditch):
    ground_type = ground["amount_type"]
    ditch_type = ditch["amount_type"]
    ...
    return {
        "name": f"Both_{ground['name']}_{ditch['name']}",
        "asset_name": f"PCG_Cubeless_ED_ProfileMatrix_Both_{ground['name']}_{ditch['name']}",
        "profile_mode": profile_mode,
        "ground_amount_type": ground_type,
        "ditch_amount_type": ditch_type,
        "profile_matrix_type": matrix_type,
        "profile_matrix_id": profile_matrix_id(profile_mode, ground_type, ditch_type),
        "subgraph_path": matrix_graph_path(ground, ditch),
...
PROFILE_MATRIX_SPECS = (
    [make_ground_only_spec(ground) for ground in GROUND_AMOUNT_SPECS]
    + [make_ditch_only_spec(ditch) for ditch in DITCH_AMOUNT_SPECS]
    + [
        make_both_spec(ground, ditch)
        for ground in GROUND_AMOUNT_SPECS
        for ditch in DITCH_AMOUNT_SPECS
    ]
)

PROFILE_MATRIX_GRAPH_PATHS = {
    spec["name"]: f"{PACKAGE_PATH}/{spec['asset_name']}.{spec['asset_name']}"
    for spec in PROFILE_MATRIX_SPECS
}
...
def add_profile_matrix_markers(graph, upstream, spec):
    markers = [
    ...
    for index, (attr_name, value_type, value) in enumerate(markers):
        current = add_marker(
            graph,
            current,
            f"ProfileMatrix {spec['profile_matrix_id']} {attr_name}",
            attr_name,
            value_type,
            value,
            -160 + index * 340,
...
def build_profile_matrix_graph(spec):
    unreal.EditorAssetLibrary.make_directory(PACKAGE_PATH)
    graph_path = PROFILE_MATRIX_GRAPH_PATHS[spec["name"]]
    ...
    if not graph:
        raise RuntimeError(f"Failed to create/load profile matrix graph: {spec['name']}")
    ...
    subgraph = add_node(
        ...
        f"{spec['name']} Source Subgraph",
        -1180,
        0,
    )
    configure_subgraph(subgraph, spec["subgraph_path"])
    graph.add_edge(graph.get_input_node(), "In", subgraph, "In")
    final = add_profile_matrix_markers(graph, subgraph, spec)
...
def spawn_validation_actor(spec, graph, index):
    ...
    actor = unreal.EditorLevelLibrary.spawn_actor_from_class(
        actor_class,
        unreal.Vector(21800 + column * 560, row * 520, 0),
...
def main():
    print("MCP_CUBELESS_ED_PROFILE_MATRIX_PRESETS_BUILD_BEGIN")
    unreal.get_editor_subsystem(unreal.LevelEditorSubsystem).load_level(LEVEL)
    for index, spec in enumerate(PROFILE_MATRIX_SPECS):
        graph = build_profile_matrix_graph(spec)
        actor = spawn_validation_actor(spec, graph, index)
        print(f"profile_matrix={spec['name']}")
        print(f"profile_matrix_graph={graph.get_path_name()}")
        print(f"validation_actor={actor.get_actor_label()}")
        print(f"profile_mode={spec['profile_mode']}")
        print(f"profile_matrix_id={spec['profile_matrix_id']}")
        print(f"profile_matrix_type={spec['profile_matrix_type']}")
    ...
    print("MCP_CUBELESS_ED_PROFILE_MATRIX_PRESETS_BUILD_END")
