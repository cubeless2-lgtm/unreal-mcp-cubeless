...
BLUEPRINT_NAME = "BP_Cubeless_ED_PCGMatrixSelector"
...
DEFAULT_GRAPH = (
    ...
    "PCG_Cubeless_ED_Matrix_GroundNormal_DitchNormal.PCG_Cubeless_ED_Matrix_GroundNormal_DitchNormal"
)
...
print("MCP_CUBELESS_ED_MATRIX_SELECTOR_ASSET_SETUP_BEGIN")
...
print("MCP_CUBELESS_ED_MATRIX_SELECTOR_ASSET_SETUP_END")
"""
    execute_unreal(code)
...
print("MCP_CUBELESS_ED_MATRIX_SELECTOR_TEMPLATE_SETUP_BEGIN")
...
if not default_graph:
    raise RuntimeError(f"Missing default matrix graph: {{DEFAULT_GRAPH}}")
...
for handle in subsystem.k2_gather_subobject_data_for_blueprint(blueprint):
    ...
    if isinstance(obj, unreal.SplineComponent):
        obj.modify()
        obj.set_editor_property("component_tags", ["CubelessEDMatrixSpline"])
    if isinstance(obj, unreal.PCGComponent):
        ...
        obj.set_editor_property("component_tags", ["CubelessEDMatrixSelector"])
        configured.append((obj.get_name(), default_graph.get_path_name()))

unreal.BlueprintEditorLibrary.compile_blueprint(blueprint)
unreal.EditorAssetLibrary.save_loaded_asset(blueprint)
print(f"configured_pcg_templates={{configured}}")
print("MCP_CUBELESS_ED_MATRIX_SELECTOR_TEMPLATE_SETUP_END")
"""
    execute_unreal(code)


def add_selector_variable(name, friendly_name, tooltip):
    require_success(
...
def add_selector_variables():
    add_selector_variable(
        "GroundAmountType",
        "Ground Amount Type",
        "1 = Sparse, 2 = Normal, 3 = Dense. Apply from the Cubeless ED PCG menu after changing.",
    )
    add_selector_variable(
        "DitchAmountType",
        "Ditch Amount Type",
...
if not blueprint:
    raise RuntimeError(f"Missing matrix selector Blueprint: {{BLUEPRINT_PATH}}")
for variable_name in ("GroundAmountType", "DitchAmountType"):
...
def main():
    print("MCP_CUBELESS_ED_MATRIX_SELECTOR_BUILD_BEGIN")
    ...
    if not result.get("validation_pass"):
        raise RuntimeError(f"Matrix selector Blueprint compile validation failed: {result}")
    ...
    print("MCP_CUBELESS_ED_MATRIX_SELECTOR_BUILD_END")
