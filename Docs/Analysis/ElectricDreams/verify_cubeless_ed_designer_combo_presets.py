...
SCRIPT_DIR = pathlib.Path(
    globals().get(
        "__file__",
        pathlib.Path.cwd() / "Docs" / "Analysis" / "ElectricDreams" / "verify_cubeless_ed_designer_combo_presets.py",
    )
).resolve().parent

BUILDER_SCRIPT = str(SCRIPT_DIR / "build_cubeless_ed_designer_combo_presets.py")
VERIFY_MARKER = "MCP_CUBELESS_ED_DESIGNER_COMBO_PRESETS_VERIFY_BEGIN"


def load_builder_config():
    namespace = {"__name__": "_cubeless_ed_designer_combo_presets_config"}
...
def validate_combo(spec, config):
    graph = unreal.EditorAssetLibrary.load_asset(config["COMBO_GRAPH_PATHS"][spec["name"]])
    if not graph:
        raise RuntimeError(f"Missing combo graph: {spec['name']}")
    ...
    return {
        "combo": spec["name"],
        "actor": label,
        "graph": config["COMBO_GRAPH_PATHS"][spec["name"]],
...
def main():
    ...
    for graph_path in config["COMBO_GRAPH_PATHS"].values():
        graph = unreal.EditorAssetLibrary.load_asset(graph_path)
        if not graph:
            raise RuntimeError(f"Missing combo graph: {graph_path}")

    results = [validate_combo(spec, config) for spec in config["COMBO_SPECS"]]
    log_path, marker_found, log_errors = scan_latest_log("MCP_CUBELESS_ED_DESIGNER_COMBO_PRESETS_BUILD_BEGIN")
    validation_pass = all(result["validation_pass"] for result in results) and not log_errors

    print(VERIFY_MARKER)
    print(f"combo_graph_paths={config['COMBO_GRAPH_PATHS']}")
    ...
    print(f"designer_combo_presets_validation_pass={validation_pass}")
    print("MCP_CUBELESS_ED_DESIGNER_COMBO_PRESETS_VERIFY_END")
    if not validation_pass:
        raise RuntimeError("Cubeless ED designer combo presets verification failed")
