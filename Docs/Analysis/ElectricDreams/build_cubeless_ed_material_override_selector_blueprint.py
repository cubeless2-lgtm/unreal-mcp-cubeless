...
BLUEPRINT_NAME = "BP_Cubeless_ED_PCGMaterialOverrideSelector"
...
print("MCP_CUBELESS_ED_MATERIAL_OVERRIDE_SELECTOR_ASSET_SETUP_BEGIN")
...
print("MCP_CUBELESS_ED_MATERIAL_OVERRIDE_SELECTOR_ASSET_SETUP_END")
"""
    execute_unreal(code)
...
print("MCP_CUBELESS_ED_MATERIAL_OVERRIDE_SELECTOR_TEMPLATE_SETUP_BEGIN")
...
if not default_graph:
    raise RuntimeError(f"Missing default material override graph: {{DEFAULT_GRAPH}}")
...
for handle in subsystem.k2_gather_subobject_data_for_blueprint(blueprint):
    ...
    if isinstance(obj, unreal.SplineComponent):
        obj.modify()
        obj.set_editor_property("component_tags", ["CubelessEDMaterialOverrideSpline"])
    if isinstance(obj, unreal.PCGComponent):
        ...
        obj.set_editor_property("component_tags", ["CubelessEDMaterialOverrideSelector"])
        configured.append((obj.get_name(), default_graph.get_path_name()))

unreal.BlueprintEditorLibrary.compile_blueprint(blueprint)
unreal.EditorAssetLibrary.save_loaded_asset(blueprint)
print(f"configured_pcg_templates={{configured}}")
print("MCP_CUBELESS_ED_MATERIAL_OVERRIDE_SELECTOR_TEMPLATE_SETUP_END")
"""
    execute_unreal(code)
...
def add_selector_variable(name, default_value, friendly_name, tooltip):
    require_success(
        "add_blueprint_variable",
        {
            ...
            "is_exposed": True,
            "category": "Cubeless PCG Material Override",
...
def add_selector_variables():
    add_selector_variable(
        ...
        "1 = GroundFoliage, 2 = SmallRocks, 3 = CompactConifer. Apply from the Cubeless ED PCG menu after changing.",
    )
    add_selector_variable(
        "MaterialVariantType",
        2,
...
if not blueprint:
    raise RuntimeError(f"Missing material override selector Blueprint: {{BLUEPRINT_PATH}}")
for variable_name in ("MaterialDomainType", "MaterialVariantType"):
...
def main():
    print("MCP_CUBELESS_ED_MATERIAL_OVERRIDE_SELECTOR_BUILD_BEGIN")
    ...
    if not result.get("validation_pass"):
        raise RuntimeError(f"Material override selector Blueprint compile validation failed: {result}")
    ...
    print("MCP_CUBELESS_ED_MATERIAL_OVERRIDE_SELECTOR_BUILD_END")
