...
SCRIPT_DIR = pathlib.Path(
    globals().get(
        "__file__",
        pathlib.Path.cwd() / "Docs" / "Analysis" / "ElectricDreams" / "verify_cubeless_ed_ditch_amount_presets.py",
    )
).resolve().parent

BUILDER_SCRIPT = str(SCRIPT_DIR / "build_cubeless_ed_ditch_amount_presets.py")
VERIFY_MARKER = "MCP_CUBELESS_ED_DITCH_AMOUNT_PRESETS_VERIFY_BEGIN"


def load_builder_config():
    namespace = {"__name__": "_cubeless_ed_ditch_amount_presets_config"}
...
def validate_amount(spec, config):
    ...
    return {
        ...
        "rows_sample": sorted(rows, key=lambda item: (item["designer_amount_id"], item["seed"], item["translation"]))[:18],
    }
...
def main():
    ...
    log_path, marker_found, log_errors = scan_latest_log("MCP_CUBELESS_ED_DITCH_AMOUNT_PRESETS_BUILD_BEGIN")
    ...
    print(f"ditch_amount_presets_validation_pass={validation_pass}")
    print("MCP_CUBELESS_ED_DITCH_AMOUNT_PRESETS_VERIFY_END")
    if not validation_pass:
        raise RuntimeError("Cubeless ED ditch amount preset verification failed")
