...
PACKAGE_PATH = "/Game/Cubeless/PCG/ElectricDreamsLearning/Blueprints/DesignerCombos"
LEVEL = "/Game/_MCP_Temp/PCG/LVL_ElectricDreams_SplineAssembly_MCP"
ACTOR_LABEL_PREFIX = "MCP_Cubeless_ED_DesignerComboBlueprint"
TARGET_SAMPLE_COUNT = 6
TARGET_POINT_SPACING = 320.0


def load_combo_config():
    namespace = {"__name__": "_cubeless_ed_designer_combo_config"}
    with open(COMBO_BUILDER_SCRIPT, "r", encoding="utf-8") as handle:
        code = compile(handle.read(), str(COMBO_BUILDER_SCRIPT), "exec")
    exec(code, namespace)
    return namespace


def combo_blueprint_specs():
    config = load_combo_config()
    specs = []
    for spec in config["COMBO_SPECS"]:
        copied = dict(spec)
        copied["asset_name"] = f"BP_Cubeless_ED_DesignerCombo_{spec['name']}"
        copied["graph_path"] = config["COMBO_GRAPH_PATHS"][spec["name"]]
        specs.append(copied)
    return specs


COMBO_BLUEPRINT_SPECS = combo_blueprint_specs()
...
def spawn_validation_actor(spec, graph, index):
    ...
    if not actor_class:
        raise RuntimeError(f"Missing combo Blueprint class: {blueprint_class_path(spec)}")

    actor = unreal.EditorLevelLibrary.spawn_actor_from_class(
        actor_class,
        unreal.Vector(14800 + index * 520, 0, 0),
...
def main():
    print("MCP_CUBELESS_ED_DESIGNER_COMBO_BLUEPRINTS_BUILD_BEGIN")
    unreal.get_editor_subsystem(unreal.LevelEditorSubsystem).load_level(LEVEL)
    for index, spec in enumerate(COMBO_BLUEPRINT_SPECS):
        graph = unreal.EditorAssetLibrary.load_asset(spec["graph_path"])
        if not graph:
            raise RuntimeError(f"Missing combo graph for {spec['name']}: {spec['graph_path']}")
    ...
    print("MCP_CUBELESS_ED_DESIGNER_COMBO_BLUEPRINTS_BUILD_END")
