...
SCRIPT_DIR = pathlib.Path(
    globals().get(
        "__file__",
        r"D:\Git\unreal-mcp-cubeless\Docs\Analysis\ElectricDreams\verify_cubeless_ed_profile_matrix_selector_blueprint.py",
    )
).parent
PROFILE_MATRIX_BUILDER_SCRIPT = SCRIPT_DIR / "build_cubeless_ed_profile_matrix_presets.py"
APPLY_SCRIPT = SCRIPT_DIR / "apply_cubeless_ed_profile_matrix_selector.py"

BLUEPRINT_PATH = (
    "/Game/Cubeless/PCG/ElectricDreamsLearning/Blueprints/Authoring/"
    "BP_Cubeless_ED_PCGProfileMatrixSelector.BP_Cubeless_ED_PCGProfileMatrixSelector"
)
...
ACTOR_LABEL_PREFIX = "MCP_Cubeless_ED_ProfileMatrixSelector"
TARGET_SAMPLE_COUNT = 6
TARGET_POINT_SPACING = 320.0
VERIFY_MARKER = "MCP_CUBELESS_ED_PROFILE_MATRIX_SELECTOR_VERIFY_BEGIN"
VALIDATION_MODE = globals().get("VALIDATION_MODE", "verify")


def load_profile_matrix_config():
    namespace = {"__name__": "_cubeless_ed_profile_matrix_presets_config"}
    with open(PROFILE_MATRIX_BUILDER_SCRIPT, "r", encoding="utf-8") as handle:
        code = compile(handle.read(), str(PROFILE_MATRIX_BUILDER_SCRIPT), "exec")
    exec(code, namespace)
    return namespace


def load_apply_module():
    namespace = {"__name__": "_cubeless_ed_profile_matrix_selector_apply", "__file__": str(APPLY_SCRIPT)}
...
def configure_validation_spline(actor):
    ...
    if not splines:
        raise RuntimeError("Profile matrix selector actor has no SplineComponent")
...
def spawn_selector_actor(spec, index, apply_module):
    ...
    if not actor_class:
        raise RuntimeError(f"Missing profile matrix selector Blueprint class: {BLUEPRINT_CLASS_PATH}")

    row = index // 5
    column = index % 5
    actor = unreal.EditorLevelLibrary.spawn_actor_from_class(
        actor_class,
        unreal.Vector(21400 + column * 560, row * 520, 0),
    ...
    apply_result = apply_module["apply_profile_matrix_selector"](actor, force=True)
...
def validate_selector(spec, config):
    ...
    if not actor:
        raise RuntimeError(
            f"Missing prepared profile matrix selector validation actor: {label}. "
            "Run prepare_cubeless_ed_profile_matrix_selector_validation.py first."
        )

    components = actor.get_components_by_class(unreal.PCGComponent)
    if not components:
        raise RuntimeError(f"Profile matrix selector actor has no PCGComponent: {label}")
    ...
    expected_graph_path = config["PROFILE_MATRIX_GRAPH_PATHS"][spec["name"]]
...
def main():
    ...
    if not blueprint:
        raise RuntimeError(f"Missing profile matrix selector Blueprint: {BLUEPRINT_PATH}")

    ensure_validation_level_loaded()
    config = load_profile_matrix_config()
    apply_module = load_apply_module()
    specs = config["PROFILE_MATRIX_SPECS"]

    if VALIDATION_MODE == "prepare":
        results = [spawn_selector_actor(spec, index, apply_module) for index, spec in enumerate(specs)]
        print(f"profile_matrix_selector_blueprint={BLUEPRINT_PATH}")
        print(f"profile_matrix_selector_class={BLUEPRINT_CLASS_PATH}")
        for actor, apply_result in results:
            print(f"prepared_actor={actor.get_actor_label()}")
            print(f"  apply_result={apply_result}")
        print("profile_matrix_selector_validation_prepared=True")
        print("MCP_CUBELESS_ED_PROFILE_MATRIX_SELECTOR_VERIFY_END")
        return
    ...
    print(f"profile_matrix_selector_validation_pass={validation_pass}")
    print("MCP_CUBELESS_ED_PROFILE_MATRIX_SELECTOR_VERIFY_END")
    if not validation_pass:
        raise RuntimeError("Cubeless ED profile matrix selector verification failed")
