...
This contract validates a future authoring-open-only record after the durable
authoring activation readiness record is valid. It does not activate or open
the executor, start authoring enablement, enable durable authoring, complete
...
DURABLE_EXECUTOR_AUTHORING_OPEN_SCHEMA = (
    "section_127_durable_executor_authoring_open_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_OPEN_RECORD_SCHEMA = (
    "section_127_durable_executor_authoring_open_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_OPEN_SUMMARY_SCHEMA = (
    "section_127_durable_executor_authoring_open_summary_v1"
)
EXPECTED_OPEN_SCOPE = "durable_executor_authoring_open_only"
...
def build_durable_executor_authoring_open_contract(
    requested: bool,
    ...
    activation_readiness_contract_ready = bool(
        ...
        == 1
        and activation_readiness_summary.get("activation_readiness_record_rejected_count")
        == 0
        and activation_readiness_summary.get("unsafe_activation_readiness_record_count")
        == 0
        and activation_readiness_summary.get(
    ...
    activation_readiness_inputs_satisfied = bool(
        activation_readiness_summary.get("activation_readiness_inputs_satisfied_count")
        == 1
    )
    activation_readiness_record_valid = bool(
        activation_readiness_summary.get("activation_readiness_record_valid_count") == 1
    )
    allowed_activation_readiness_observed = bool(
        activation_readiness_summary.get("allowed_activation_readiness_observed_count")
        == 1
    )
    no_forbidden_activation_readiness_claims = bool(
        activation_readiness_summary.get("no_forbidden_activation_readiness_claims_count")
        == 1
    )
    ...
    record_schema_matches = bool(
        open_record_present
        and open_record.get("schema") == DURABLE_EXECUTOR_AUTHORING_OPEN_RECORD_SCHEMA
    )
    ...
    if requested:
        if not activation_readiness_inputs_satisfied:
            missing.append("section_126_activation_readiness_inputs_satisfied")
        if not activation_readiness_record_valid:
            missing.append("section_126_activation_readiness_record_valid")
        if not allowed_activation_readiness_observed:
            missing.append("section_126_allowed_activation_readiness_observed")
        if not no_forbidden_activation_readiness_claims:
            missing.append("section_126_no_forbidden_activation_readiness_claims")
        ...
        if not open_scope_matches:
            missing.append("durable_executor_authoring_open_only_scope")
    ...
    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_open",
        "schema": DURABLE_EXECUTOR_AUTHORING_OPEN_SCHEMA,
        "requested": requested,
        "open_contract_defined": open_contract_defined,
        "required_open_record_schema": (
            DURABLE_EXECUTOR_AUTHORING_OPEN_RECORD_SCHEMA if requested else ""
        ),
...
def summarize_durable_executor_authoring_opens(
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_OPEN_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_open_count": len(requested),
