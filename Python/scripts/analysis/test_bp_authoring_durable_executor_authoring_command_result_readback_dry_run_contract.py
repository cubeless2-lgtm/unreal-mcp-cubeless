#!/usr/bin/env python
"""Offline smoke tests for Section 170 command result readback dry-run contract."""
...
import bp_authoring_durable_executor_authoring_command_completion_result_dry_run_contract as result  # noqa: E402
import bp_authoring_durable_executor_authoring_command_result_readback_dry_run_contract as readback  # noqa: E402


def build_section_169_summary(**overrides: object) -> dict:
    summary = {
        "schema": result.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_RESULT_DRY_RUN_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_result_readback_record(**overrides: object) -> dict:
    record = {
        "schema": readback.DURABLE_EXECUTOR_AUTHORING_COMMAND_RESULT_READBACK_DRY_RUN_RECORD_SCHEMA,
        "result_readback_scope": readback.EXPECTED_RESULT_READBACK_SCOPE,
        "status": "passed",
        "dry_run_only": True,
        "requested_command": "create_blueprint_asset",
        "result_readback_operation": "result_readback_dry_run_only",
...
def main() -> int:
    current_summary = build_section_169_summary()
    contract = readback.build_durable_executor_authoring_command_result_readback_dry_run_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == readback.DURABLE_EXECUTOR_AUTHORING_COMMAND_RESULT_READBACK_DRY_RUN_SCHEMA
    assert contract["requested"] is True
    assert contract["result_readback_contract_defined"] is True
    assert contract["section_169_completion_result_contract_ready"] is True
    assert contract["completion_result_chain_satisfied"] is False
    assert contract["result_readback_dry_run_record_present"] is False
    assert contract["result_readback_dry_run_record_valid"] is False
    assert contract["result_readback_dry_run_record_rejected"] is False
    assert contract["result_readback_dry_run_admissible"] is False
    assert contract["missing_result_readback_dry_run_prerequisite_count"] == 35
    assert "section_169_completion_result_dry_run_admissible" in contract[
        "missing_result_readback_dry_run_prerequisites"
    ]
    assert "command_result_readback_dry_run_record_present" in contract[
        "missing_result_readback_dry_run_prerequisites"
    ]
    assert contract["durable_result_readback_promoted"] is False
    assert contract["durable_result_readback_accepted"] is False
    assert contract["durable_completion_result_promoted"] is False
    assert contract["durable_completion_result_recorded"] is False
    ...
    summary = readback.summarize_durable_executor_authoring_command_result_readback_dry_runs(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_command_result_readback_dry_run_count"] == 1
    assert summary["result_readback_contract_defined_count"] == 1
    assert summary["section_169_completion_result_contract_ready_count"] == 1
    assert summary["completion_result_chain_satisfied_count"] == 0
    assert summary["result_readback_dry_run_record_present_count"] == 0
    assert summary["result_readback_dry_run_record_valid_count"] == 0
    assert summary["result_readback_dry_run_record_rejected_count"] == 0
    assert summary["result_readback_dry_run_admissible_count"] == 0
    assert summary["durable_result_readback_promoted_count"] == 0
    assert summary["durable_result_readback_accepted_count"] == 0
    assert summary["durable_completion_result_promoted_count"] == 0
    assert summary["durable_completion_result_recorded_count"] == 0
    ...
    blocked_with_record = readback.build_durable_executor_authoring_command_result_readback_dry_run_contract(
        True,
        current_summary,
        build_result_readback_record(),
    )
    assert blocked_with_record["completion_result_chain_satisfied"] is False
    assert blocked_with_record["result_readback_dry_run_record_valid"] is False
    assert blocked_with_record["result_readback_dry_run_record_rejected"] is True
    assert blocked_with_record["result_readback_dry_run_admissible"] is False
    assert blocked_with_record["missing_result_readback_dry_run_prerequisite_count"] == 21
    assert blocked_with_record["durable_result_readback_promoted"] is False
    assert blocked_with_record["durable_result_readback_accepted"] is False
    ...
    future_summary = build_section_169_summary(
    ...
    future_contract = readback.build_durable_executor_authoring_command_result_readback_dry_run_contract(
        True,
        future_summary,
        build_result_readback_record(),
    )
    assert future_contract["completion_result_chain_satisfied"] is True
    assert future_contract["result_readback_dry_run_record_valid"] is True
    assert future_contract["result_readback_dry_run_record_rejected"] is False
    assert future_contract["result_readback_dry_run_admissible"] is True
    assert future_contract["missing_result_readback_dry_run_prerequisite_count"] == 0
    assert future_contract["durable_result_readback_promoted"] is False
    assert future_contract["durable_result_readback_accepted"] is False
    assert future_contract["durable_completion_result_promoted"] is False
    assert future_contract["durable_completion_result_recorded"] is False
    ...
    future_summary_result = readback.summarize_durable_executor_authoring_command_result_readback_dry_runs(
        [future_contract]
    )
    assert future_summary_result["status"] == "passed"
    assert future_summary_result["result_readback_dry_run_admissible_count"] == 1
    assert future_summary_result["durable_result_readback_promoted_count"] == 0
    assert future_summary_result["durable_result_readback_accepted_count"] == 0
    assert future_summary_result["durable_completion_result_promoted_count"] == 0
    assert future_summary_result["durable_completion_result_recorded_count"] == 0
    ...
    forbidden_contract = readback.build_durable_executor_authoring_command_result_readback_dry_run_contract(
        True,
        future_summary,
        build_result_readback_record(requested_command="save_asset"),
    )
    assert forbidden_contract["requested_command_forbidden"] is True
    assert forbidden_contract["result_readback_dry_run_record_rejected"] is True
    assert forbidden_contract["result_readback_dry_run_admissible"] is False
    forbidden_summary = readback.summarize_durable_executor_authoring_command_result_readback_dry_runs(
        [forbidden_contract]
    )
    ...
    unsafe_contract = readback.build_durable_executor_authoring_command_result_readback_dry_run_contract(
        True,
        future_summary,
        build_result_readback_record(durable_result_readback_accepted=True),
    )
    assert unsafe_contract["unsafe_result_readback_record_count"] == 1
    assert unsafe_contract["result_readback_dry_run_record_rejected"] is True
    assert unsafe_contract["result_readback_dry_run_admissible"] is False
    assert unsafe_contract["durable_result_readback_accepted"] is False
    ...
    print("BP authoring durable executor authoring command result readback dry-run contract smoke passed")
    return 0
