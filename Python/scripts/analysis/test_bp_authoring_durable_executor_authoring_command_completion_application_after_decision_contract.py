#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_command_completion_application_after_decision_contract.py."""
...
import bp_authoring_durable_executor_authoring_command_completion_decision_after_evidence_contract as decision  # noqa: E402


def build_current_completion_decision_after_evidence_summary() -> dict:
    return {
        "schema": decision.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_AFTER_EVIDENCE_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_application_record(**overrides: object) -> dict:
    record = {
        "schema": application.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_APPLICATION_AFTER_DECISION_RECORD_SCHEMA,
...
def main() -> int:
    current_summary = build_current_completion_decision_after_evidence_summary()
    contract = application.build_durable_executor_authoring_command_completion_application_after_decision_contract(
        True,
        current_summary,
    )
    assert (
        contract["schema"]
        == application.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_APPLICATION_AFTER_DECISION_SCHEMA
    )
    ...
    assert "section_133_evidence_ready_for_completion" in contract[
        "missing_application_prerequisites"
    ]
    assert "section_133_completion_decision_record_valid" in contract[
        "missing_application_prerequisites"
    ]
    assert "durable_authoring_command_completion_application_record_present" in contract[
        "missing_application_prerequisites"
    ]
    assert (
    ...
    assert "separate_durable_authoring_command_completion_result_contract" in contract[
        "missing_application_prerequisites"
    ]
    ...
    summary = application.summarize_durable_executor_authoring_command_completion_applications_after_decision(
        [contract]
    )
    assert summary["status"] == "passed"
    assert (
        summary[
            "durable_requested_executor_authoring_command_completion_application_after_decision_count"
        ]
        == 1
    )
    ...
    future_contract = application.build_durable_executor_authoring_command_completion_application_after_decision_contract(
        True,
        future_summary,
    ...
    unsafe_contract = application.build_durable_executor_authoring_command_completion_application_after_decision_contract(
        True,
        future_summary,
    ...
    unsafe_summary = application.summarize_durable_executor_authoring_command_completion_applications_after_decision(
        [unsafe_contract]
    )
    ...
    print(
        "BP authoring durable executor authoring command completion application after decision contract smoke passed"
    )
    return 0
