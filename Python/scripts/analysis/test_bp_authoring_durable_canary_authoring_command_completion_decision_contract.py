#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_canary_authoring_command_completion_decision_contract.py."""
...
import bp_authoring_durable_canary_authoring_command_completion_decision_contract as completion_decision  # noqa: E402
import bp_authoring_durable_canary_authoring_command_execution_evidence_contract as evidence  # noqa: E402


def build_current_evidence_summary() -> dict:
    return {
        "schema": evidence.CANARY_DURABLE_AUTHORING_COMMAND_EXECUTION_EVIDENCE_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_completion_decision_record(**overrides: object) -> dict:
    record = {
        "schema": completion_decision.CANARY_DURABLE_AUTHORING_COMMAND_COMPLETION_DECISION_RECORD_SCHEMA,
        "completion_scope": completion_decision.EXPECTED_COMPLETION_SCOPE,
        "explicit_durable_authoring_command_completion_authorized": True,
...
def main() -> int:
    ...
    contract = completion_decision.build_canary_durable_authoring_command_completion_decision_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == completion_decision.CANARY_DURABLE_AUTHORING_COMMAND_COMPLETION_DECISION_SCHEMA
    ...
    assert contract["explicit_completion_authorized"] is False
    ...
    assert contract["missing_completion_prerequisite_count"] == 9
    assert (
        "section_88_authoring_command_execution_evidence_admitted"
        in contract["missing_completion_prerequisites"]
    )
    assert "section_88_allowed_evidence_command_observed" in contract["missing_completion_prerequisites"]
    assert "section_88_no_forbidden_evidence_commands" in contract["missing_completion_prerequisites"]
    assert (
        "durable_authoring_command_completion_decision_record_present"
        in contract["missing_completion_prerequisites"]
    )
    assert (
        "durable_authoring_command_completion_decision_record_schema"
        in contract["missing_completion_prerequisites"]
    )
    assert (
        "durable_canary_authoring_command_completion_decision_only_scope"
        in contract["missing_completion_prerequisites"]
    )
    assert (
        "explicit_durable_authoring_command_completion_authorization"
        in contract["missing_completion_prerequisites"]
    )
    assert "operator_reconfirmed_no_save_delete_rename" in contract["missing_completion_prerequisites"]
    assert (
    ...
    summary = completion_decision.summarize_canary_durable_authoring_command_completion_decisions(
        [contract]
    )
    assert summary == {
        "schema": completion_decision.CANARY_DURABLE_AUTHORING_COMMAND_COMPLETION_DECISION_SUMMARY_SCHEMA,
        "status": "passed",
    ...
    future_contract = completion_decision.build_canary_durable_authoring_command_completion_decision_contract(
        True,
        future_summary,
    ...
    unsafe_contract = completion_decision.build_canary_durable_authoring_command_completion_decision_contract(
        True,
        future_summary,
    ...
    unsafe_summary = completion_decision.summarize_canary_durable_authoring_command_completion_decisions(
        [unsafe_contract]
    )
    ...
    print("BP authoring durable canary authoring command completion decision contract smoke passed")
    return 0
