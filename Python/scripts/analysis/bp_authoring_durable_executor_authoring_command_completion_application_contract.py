...
assets, delete/rename, cleanup, or open durable authoring.
"""
...
DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_APPLICATION_SCHEMA = (
    "section_118_durable_executor_authoring_command_completion_application_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_APPLICATION_RECORD_SCHEMA = (
    "section_118_durable_executor_authoring_command_completion_application_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_APPLICATION_SUMMARY_SCHEMA = (
    "section_118_durable_executor_authoring_command_completion_application_summary_v1"
)
EXPECTED_APPLICATION_SCOPE = (
    "durable_executor_authoring_command_completion_application_only"
)
...
def build_durable_executor_authoring_command_completion_application_contract(
    requested: bool,
    ...
    completion_decision_contract_ready = bool(
        ...
        and completion_decision_summary.get(
            "completion_decision_contract_defined_count"
        )
        == 1
        and completion_decision_summary.get("completion_decision_record_rejected_count")
        == 0
        and completion_decision_summary.get(
        ...
        == 0
        and all(
            completion_decision_summary.get(key) == 0
    ...
    if requested:
        if not evidence_ready_for_completion:
            missing.append("section_117_evidence_ready_for_completion")
        if not completion_decision_record_valid:
            missing.append("section_117_completion_decision_record_valid")
        ...
        if not application_scope_matches:
            missing.append(
                "durable_executor_authoring_command_completion_application_only_scope"
            )
    ...
    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_command_completion_application",
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_APPLICATION_SCHEMA,
        "requested": requested,
...
def summarize_durable_executor_authoring_command_completion_applications(
    contracts: Sequence[Dict[str, Any]]
) -> Dict[str, Any]:
    requested = [contract for contract in contracts if contract.get("requested")]
    rejected_count = sum(
        1 for contract in requested if contract.get("application_record_rejected")
    )
    ...
    if requested:
        status = (
            "passed"
            if _sum_truthy(requested, "application_contract_defined")
            == len(requested)
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_APPLICATION_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_command_completion_application_count": len(
            requested
        ),
