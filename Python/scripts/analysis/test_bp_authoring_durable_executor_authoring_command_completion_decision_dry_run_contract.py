#!/usr/bin/env python
"""Offline smoke tests for Section 167 command completion decision dry-run contract."""
...
def build_section_166_summary(**overrides: object) -> dict:
    summary = {
        "schema": evidence.DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_EVIDENCE_DRY_RUN_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_completion_decision_record(**overrides: object) -> dict:
    record = {
        "schema": decision.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_DRY_RUN_RECORD_SCHEMA,
        "completion_decision_scope": decision.EXPECTED_COMPLETION_DECISION_SCOPE,
        "status": "passed",
        "dry_run_only": True,
        "requested_command": "create_blueprint_asset",
        "completion_decision_operation": "completion_decision_dry_run_only",
...
def main() -> int:
    current_summary = build_section_166_summary()
    contract = decision.build_durable_executor_authoring_command_completion_decision_dry_run_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == decision.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_DRY_RUN_SCHEMA
    assert contract["requested"] is True
    assert contract["completion_decision_contract_defined"] is True
    assert contract["section_166_execution_evidence_contract_ready"] is True
    assert contract["execution_evidence_chain_satisfied"] is False
    assert contract["completion_decision_dry_run_record_present"] is False
    assert contract["completion_decision_dry_run_record_valid"] is False
    assert contract["completion_decision_dry_run_record_rejected"] is False
    assert contract["completion_decision_dry_run_admissible"] is False
    assert contract["missing_completion_decision_dry_run_prerequisite_count"] == 29
    assert "section_166_execution_evidence_dry_run_admissible" in contract[
        "missing_completion_decision_dry_run_prerequisites"
    ]
    assert "command_completion_decision_dry_run_record_present" in contract[
        "missing_completion_decision_dry_run_prerequisites"
    ]
    ...
    summary = decision.summarize_durable_executor_authoring_command_completion_decision_dry_runs(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_command_completion_decision_dry_run_count"] == 1
    assert summary["completion_decision_contract_defined_count"] == 1
    assert summary["section_166_execution_evidence_contract_ready_count"] == 1
    assert summary["execution_evidence_chain_satisfied_count"] == 0
    assert summary["completion_decision_dry_run_record_present_count"] == 0
    assert summary["completion_decision_dry_run_record_valid_count"] == 0
    assert summary["completion_decision_dry_run_record_rejected_count"] == 0
    assert summary["completion_decision_dry_run_admissible_count"] == 0
    ...
    blocked_with_record = decision.build_durable_executor_authoring_command_completion_decision_dry_run_contract(
        True,
        current_summary,
        build_completion_decision_record(),
    )
    assert blocked_with_record["execution_evidence_chain_satisfied"] is False
    assert blocked_with_record["completion_decision_dry_run_record_valid"] is False
    assert blocked_with_record["completion_decision_dry_run_record_rejected"] is True
    assert blocked_with_record["completion_decision_dry_run_admissible"] is False
    assert blocked_with_record["missing_completion_decision_dry_run_prerequisite_count"] == 15
    assert blocked_with_record["durable_completion_decision_promoted"] is False
    ...
    future_summary = build_section_166_summary(
    ...
    future_contract = decision.build_durable_executor_authoring_command_completion_decision_dry_run_contract(
        True,
        future_summary,
        build_completion_decision_record(),
    )
    assert future_contract["execution_evidence_chain_satisfied"] is True
    assert future_contract["completion_decision_dry_run_record_valid"] is True
    assert future_contract["completion_decision_dry_run_record_rejected"] is False
    assert future_contract["completion_decision_dry_run_admissible"] is True
    assert future_contract["missing_completion_decision_dry_run_prerequisite_count"] == 0
    ...
    future_summary_result = decision.summarize_durable_executor_authoring_command_completion_decision_dry_runs(
        [future_contract]
    )
    assert future_summary_result["status"] == "passed"
    assert future_summary_result["completion_decision_dry_run_admissible_count"] == 1
    ...
    forbidden_contract = decision.build_durable_executor_authoring_command_completion_decision_dry_run_contract(
        True,
        future_summary,
        build_completion_decision_record(requested_command="save_asset"),
    )
    assert forbidden_contract["requested_command_forbidden"] is True
    assert forbidden_contract["completion_decision_dry_run_record_rejected"] is True
    assert forbidden_contract["completion_decision_dry_run_admissible"] is False
    forbidden_summary = decision.summarize_durable_executor_authoring_command_completion_decision_dry_runs(
        [forbidden_contract]
    )
    ...
    unsafe_contract = decision.build_durable_executor_authoring_command_completion_decision_dry_run_contract(
        True,
        future_summary,
        build_completion_decision_record(durable_authoring_command_completed=True),
    )
    assert unsafe_contract["unsafe_completion_decision_record_count"] == 1
    assert unsafe_contract["completion_decision_dry_run_record_rejected"] is True
    assert unsafe_contract["completion_decision_dry_run_admissible"] is False
    ...
    print("BP authoring durable executor authoring command completion decision dry-run contract smoke passed")
    return 0
