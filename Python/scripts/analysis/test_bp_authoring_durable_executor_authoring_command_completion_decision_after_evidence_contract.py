#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_command_completion_decision_after_evidence_contract.py."""
...
import bp_authoring_durable_executor_authoring_command_completion_decision_after_evidence_contract as completion_decision  # noqa: E402
import bp_authoring_durable_executor_authoring_command_execution_evidence_after_execution_contract as evidence  # noqa: E402


def build_current_evidence_after_execution_summary() -> dict:
    return {
        "schema": evidence.DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_EVIDENCE_AFTER_EXECUTION_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_completion_decision_record(**overrides: object) -> dict:
    record = {
        "schema": completion_decision.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_AFTER_EVIDENCE_RECORD_SCHEMA,
...
def main() -> int:
    current_summary = build_current_evidence_after_execution_summary()
    contract = completion_decision.build_durable_executor_authoring_command_completion_decision_after_evidence_contract(
        True,
        current_summary,
    )
    assert (
        contract["schema"]
        == completion_decision.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_AFTER_EVIDENCE_SCHEMA
    )
    ...
    assert "section_132_authoring_command_execution_evidence_admitted" in contract[
        "missing_completion_prerequisites"
    ]
    assert "section_132_allowed_evidence_command_observed" in contract[
        "missing_completion_prerequisites"
    ]
    assert "section_132_no_forbidden_evidence_commands" in contract[
        "missing_completion_prerequisites"
    ]
    ...
    summary = completion_decision.summarize_durable_executor_authoring_command_completion_decisions_after_evidence(
        [contract]
    )
    assert summary["status"] == "passed"
    assert (
        summary[
            "durable_requested_executor_authoring_command_completion_decision_after_evidence_count"
        ]
        == 1
    )
    ...
    future_contract = completion_decision.build_durable_executor_authoring_command_completion_decision_after_evidence_contract(
        True,
        future_summary,
    ...
    unsafe_contract = completion_decision.build_durable_executor_authoring_command_completion_decision_after_evidence_contract(
        True,
        future_summary,
    ...
    unsafe_summary = completion_decision.summarize_durable_executor_authoring_command_completion_decisions_after_evidence(
        [unsafe_contract]
    )
    ...
    action_claim = completion_decision.build_durable_executor_authoring_command_completion_decision_after_evidence_contract(
        True,
        future_summary,
    ...
    print(
        "BP authoring durable executor authoring command completion decision after evidence contract smoke passed"
    )
    return 0
