#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_final_no_save_release_contract.py."""
...
def build_current_readback_summary() -> dict:
    return {
        "schema": readback.DURABLE_EXECUTOR_AUTHORING_COMMAND_RESULT_READBACK_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_final_no_save_release_record(**overrides: object) -> dict:
    record = {
        "schema": final_release.DURABLE_EXECUTOR_AUTHORING_FINAL_NO_SAVE_RELEASE_RECORD_SCHEMA,
...
def main() -> int:
    ...
    contract = final_release.build_durable_executor_authoring_final_no_save_release_contract(
        True,
        current_summary,
    )
    assert (
        contract["schema"]
        == final_release.DURABLE_EXECUTOR_AUTHORING_FINAL_NO_SAVE_RELEASE_SCHEMA
    )
    ...
    assert "section_120_readback_inputs_satisfied" in contract[
        "missing_final_no_save_release_prerequisites"
    ]
    assert "section_120_readback_record_valid" in contract[
        "missing_final_no_save_release_prerequisites"
    ]
    assert "section_120_allowed_readback_observed" in contract[
        "missing_final_no_save_release_prerequisites"
    ]
    assert "section_120_no_forbidden_readbacks" in contract[
        "missing_final_no_save_release_prerequisites"
    ]
    ...
    summary = final_release.summarize_durable_executor_authoring_final_no_save_releases(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_final_no_save_release_count"] == 1
    ...
    future_contract = (
        final_release.build_durable_executor_authoring_final_no_save_release_contract(
            True,
            future_summary,
    ...
    unsafe_summary = (
        final_release.summarize_durable_executor_authoring_final_no_save_releases(
            [unsafe_contract]
        )
    )
    ...
    print("BP authoring durable executor authoring final no-save release contract smoke passed")
    return 0
