#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_command_completion_application_contract.py."""
...
import bp_authoring_durable_executor_authoring_command_completion_application_contract as application  # noqa: E402
import bp_authoring_durable_executor_authoring_command_completion_decision_contract as decision  # noqa: E402


def build_current_completion_decision_summary() -> dict:
    return {
        "schema": (
            decision.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_SUMMARY_SCHEMA
        ),
        "status": "passed",
...
def build_application_record(**overrides: object) -> dict:
    record = {
        "schema": (
            application.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_APPLICATION_RECORD_SCHEMA
        ),
...
def main() -> int:
    ...
    contract = (
        application.build_durable_executor_authoring_command_completion_application_contract(
            True,
            current_summary,
        )
    )
    assert contract["schema"] == application.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_APPLICATION_SCHEMA
    ...
    assert contract["application_status_passed"] is False
    ...
    assert "section_117_evidence_ready_for_completion" in contract[
        "missing_application_prerequisites"
    ]
    assert "section_117_completion_decision_record_valid" in contract[
        "missing_application_prerequisites"
    ]
    assert "durable_authoring_command_completion_application_record_present" in contract[
        "missing_application_prerequisites"
    ]
    assert "durable_executor_authoring_command_completion_application_only_scope" in contract[
        "missing_application_prerequisites"
    ]
    assert "explicit_durable_authoring_command_completion_application_authorization" in contract[
        "missing_application_prerequisites"
    ]
    assert "durable_authoring_command_completion_application_status_passed" in contract[
        "missing_application_prerequisites"
    ]
    ...
    summary = (
        application.summarize_durable_executor_authoring_command_completion_applications(
            [contract]
        )
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_command_completion_application_count"] == 1
    ...
    print(
        "BP authoring durable executor authoring command completion application contract smoke passed"
    )
    return 0
