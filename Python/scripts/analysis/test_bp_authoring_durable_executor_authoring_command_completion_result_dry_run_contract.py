#!/usr/bin/env python
"""Offline smoke tests for Section 169 command completion result dry-run contract."""
...
def build_section_168_summary(**overrides: object) -> dict:
    summary = {
        "schema": application.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_APPLICATION_DRY_RUN_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_completion_result_record(**overrides: object) -> dict:
    record = {
        "schema": result.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_RESULT_DRY_RUN_RECORD_SCHEMA,
        "completion_result_scope": result.EXPECTED_COMPLETION_RESULT_SCOPE,
        "status": "passed",
        "dry_run_only": True,
        "requested_command": "create_blueprint_asset",
        "completion_result_operation": "completion_result_dry_run_only",
...
def main() -> int:
    current_summary = build_section_168_summary()
    contract = result.build_durable_executor_authoring_command_completion_result_dry_run_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == result.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_RESULT_DRY_RUN_SCHEMA
    assert contract["requested"] is True
    assert contract["completion_result_contract_defined"] is True
    assert contract["section_168_completion_application_contract_ready"] is True
    assert contract["completion_application_chain_satisfied"] is False
    assert contract["completion_result_dry_run_record_present"] is False
    assert contract["completion_result_dry_run_record_valid"] is False
    assert contract["completion_result_dry_run_record_rejected"] is False
    assert contract["completion_result_dry_run_admissible"] is False
    assert contract["missing_completion_result_dry_run_prerequisite_count"] == 33
    assert "section_168_completion_application_dry_run_admissible" in contract[
        "missing_completion_result_dry_run_prerequisites"
    ]
    assert "command_completion_result_dry_run_record_present" in contract[
        "missing_completion_result_dry_run_prerequisites"
    ]
    ...
    summary = result.summarize_durable_executor_authoring_command_completion_result_dry_runs(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_command_completion_result_dry_run_count"] == 1
    assert summary["completion_result_contract_defined_count"] == 1
    assert summary["section_168_completion_application_contract_ready_count"] == 1
    assert summary["completion_application_chain_satisfied_count"] == 0
    assert summary["completion_result_dry_run_record_present_count"] == 0
    assert summary["completion_result_dry_run_record_valid_count"] == 0
    assert summary["completion_result_dry_run_record_rejected_count"] == 0
    assert summary["completion_result_dry_run_admissible_count"] == 0
    ...
    blocked_with_record = result.build_durable_executor_authoring_command_completion_result_dry_run_contract(
        True,
        current_summary,
        build_completion_result_record(),
    )
    assert blocked_with_record["completion_application_chain_satisfied"] is False
    assert blocked_with_record["completion_result_dry_run_record_valid"] is False
    assert blocked_with_record["completion_result_dry_run_record_rejected"] is True
    assert blocked_with_record["completion_result_dry_run_admissible"] is False
    assert blocked_with_record["missing_completion_result_dry_run_prerequisite_count"] == 19
    assert blocked_with_record["durable_completion_result_promoted"] is False
    assert blocked_with_record["durable_completion_result_recorded"] is False
    ...
    future_summary = build_section_168_summary(
    ...
    future_contract = result.build_durable_executor_authoring_command_completion_result_dry_run_contract(
        True,
        future_summary,
        build_completion_result_record(),
    )
    assert future_contract["completion_application_chain_satisfied"] is True
    assert future_contract["completion_result_dry_run_record_valid"] is True
    assert future_contract["completion_result_dry_run_record_rejected"] is False
    assert future_contract["completion_result_dry_run_admissible"] is True
    assert future_contract["missing_completion_result_dry_run_prerequisite_count"] == 0
    ...
    future_summary_result = result.summarize_durable_executor_authoring_command_completion_result_dry_runs(
        [future_contract]
    )
    assert future_summary_result["status"] == "passed"
    assert future_summary_result["completion_result_dry_run_admissible_count"] == 1
    ...
    forbidden_contract = result.build_durable_executor_authoring_command_completion_result_dry_run_contract(
        True,
        future_summary,
        build_completion_result_record(requested_command="save_asset"),
    )
    assert forbidden_contract["requested_command_forbidden"] is True
    assert forbidden_contract["completion_result_dry_run_record_rejected"] is True
    assert forbidden_contract["completion_result_dry_run_admissible"] is False
    forbidden_summary = result.summarize_durable_executor_authoring_command_completion_result_dry_runs(
        [forbidden_contract]
    )
    ...
    unsafe_contract = result.build_durable_executor_authoring_command_completion_result_dry_run_contract(
        True,
        future_summary,
        build_completion_result_record(durable_completion_result_recorded=True),
    )
    assert unsafe_contract["unsafe_completion_result_record_count"] == 1
    assert unsafe_contract["completion_result_dry_run_record_rejected"] is True
    assert unsafe_contract["completion_result_dry_run_admissible"] is False
    assert unsafe_contract["durable_completion_result_recorded"] is False
    ...
    print("BP authoring durable executor authoring command completion result dry-run contract smoke passed")
    return 0
