#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_final_release_readiness_contract.py."""
...
import bp_authoring_durable_executor_authoring_final_no_save_release_contract as final_release  # noqa: E402
...
def build_current_final_no_save_release_summary() -> dict:
    return {
        "schema": final_release.DURABLE_EXECUTOR_AUTHORING_FINAL_NO_SAVE_RELEASE_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_readiness_record(**overrides: object) -> dict:
    record = {
        "schema": readiness.DURABLE_EXECUTOR_AUTHORING_FINAL_RELEASE_READINESS_RECORD_SCHEMA,
...
def main() -> int:
    ...
    contract = readiness.build_durable_executor_authoring_final_release_readiness_contract(
        True,
        current_summary,
    )
    assert (
        contract["schema"]
        == readiness.DURABLE_EXECUTOR_AUTHORING_FINAL_RELEASE_READINESS_SCHEMA
    )
    ...
    assert "section_121_final_no_save_release_inputs_satisfied" in contract[
        "missing_final_release_readiness_prerequisites"
    ]
    assert "section_121_final_no_save_release_record_valid" in contract[
        "missing_final_release_readiness_prerequisites"
    ]
    assert "section_121_allowed_final_no_save_release_observed" in contract[
        "missing_final_release_readiness_prerequisites"
    ]
    assert "section_121_no_forbidden_final_no_save_releases" in contract[
        "missing_final_release_readiness_prerequisites"
    ]
    ...
    assert "separate_durable_authoring_release_review_contract" in contract[
        "missing_final_release_readiness_prerequisites"
    ]
    ...
    summary = readiness.summarize_durable_executor_authoring_final_release_readiness(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_final_release_readiness_count"] == 1
    ...
    future_contract = (
        readiness.build_durable_executor_authoring_final_release_readiness_contract(
            True,
            future_summary,
    ...
    assert future_contract["missing_final_release_readiness_prerequisites"] == [
        "separate_durable_authoring_release_review_contract"
    ]
    ...
    unsafe_summary = readiness.summarize_durable_executor_authoring_final_release_readiness(
        [unsafe_contract]
    )
    ...
    code_change_summary = (
        readiness.summarize_durable_executor_authoring_final_release_readiness(
            [code_change_contract]
        )
    )
    ...
    print("BP authoring durable executor authoring final release readiness contract smoke passed")
    return 0
