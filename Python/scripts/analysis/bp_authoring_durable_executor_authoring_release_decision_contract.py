...
authoring release review record is valid. It does not accept release decision
execution, promotion barrier start, durable authoring readiness, completion,
...
DURABLE_EXECUTOR_AUTHORING_RELEASE_DECISION_SCHEMA = (
    "section_124_durable_executor_authoring_release_decision_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_RELEASE_DECISION_RECORD_SCHEMA = (
    "section_124_durable_executor_authoring_release_decision_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_RELEASE_DECISION_SUMMARY_SCHEMA = (
    "section_124_durable_executor_authoring_release_decision_summary_v1"
)
EXPECTED_RELEASE_DECISION_SCOPE = "durable_executor_authoring_release_decision_only"
...
def build_durable_executor_authoring_release_decision_contract(
    requested: bool,
    release_review_summary: Dict[str, Any],
    ...
    release_review_contract_ready = bool(
        requested
        and release_review_summary.get("status") == "passed"
        and release_review_summary.get("release_review_contract_defined_count") == 1
        and release_review_summary.get("release_review_record_rejected_count") == 0
        and release_review_summary.get("unsafe_release_review_record_count") == 0
        and release_review_summary.get("reported_forbidden_release_review_count") == 0
        and all(release_review_summary.get(key) == 0 for key in PREVIOUS_ZERO_COUNT_KEYS)
    )
    release_review_inputs_satisfied = bool(
        release_review_summary.get("release_review_inputs_satisfied_count") == 1
    )
    release_review_record_valid = bool(
        release_review_summary.get("release_review_record_valid_count") == 1
    )
    allowed_release_review_observed = bool(
        release_review_summary.get("allowed_release_review_observed_count") == 1
    )
    no_forbidden_release_review_claims = bool(
        release_review_summary.get("no_forbidden_release_review_claims_count") == 1
    )
    ...
    if requested:
        if not release_review_inputs_satisfied:
            missing.append("section_123_release_review_inputs_satisfied")
        if not release_review_record_valid:
            missing.append("section_123_release_review_record_valid")
        if not allowed_release_review_observed:
            missing.append("section_123_allowed_release_review_observed")
        if not no_forbidden_release_review_claims:
            missing.append("section_123_no_forbidden_release_review_claims")
        ...
        if not release_decision_scope_matches:
            missing.append("durable_executor_authoring_release_decision_only_scope")
        ...
        missing.append("separate_durable_authoring_release_promotion_barrier_contract")
    release_decision_record_rejected = bool(
        decision_record_present and not release_decision_record_valid
    )
    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_release_decision",
        "schema": DURABLE_EXECUTOR_AUTHORING_RELEASE_DECISION_SCHEMA,
        "requested": requested,
...
def summarize_durable_executor_authoring_release_decisions(
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_RELEASE_DECISION_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_release_decision_count": len(requested),
