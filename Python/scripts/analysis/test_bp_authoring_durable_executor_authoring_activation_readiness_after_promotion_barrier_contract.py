#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_activation_readiness_after_promotion_barrier_contract.py."""
...
def build_current_release_promotion_barrier_after_decision_summary() -> dict:
    return {
        "schema": barrier.DURABLE_EXECUTOR_AUTHORING_RELEASE_PROMOTION_BARRIER_AFTER_DECISION_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_activation_readiness_record(**overrides: object) -> dict:
    record = {
        "schema": readiness.DURABLE_EXECUTOR_AUTHORING_ACTIVATION_READINESS_AFTER_PROMOTION_BARRIER_RECORD_SCHEMA,
...
def main() -> int:
    ...
    contract = readiness.build_durable_executor_authoring_activation_readiness_after_promotion_barrier_contract(
        True,
        current_summary,
    )
    assert (
        contract["schema"]
        == readiness.DURABLE_EXECUTOR_AUTHORING_ACTIVATION_READINESS_AFTER_PROMOTION_BARRIER_SCHEMA
    )
    ...
    assert "section_141_release_promotion_barrier_inputs_satisfied" in contract[
        "missing_activation_readiness_prerequisites"
    ]
    assert "section_141_release_promotion_barrier_record_valid" in contract[
        "missing_activation_readiness_prerequisites"
    ]
    assert "section_141_allowed_release_promotion_barrier_observed" in contract[
        "missing_activation_readiness_prerequisites"
    ]
    assert "section_141_no_forbidden_release_promotion_barrier_claims" in contract[
        "missing_activation_readiness_prerequisites"
    ]
    ...
    assert "separate_durable_executor_authoring_open_after_activation_readiness_contract" in contract[
        "missing_activation_readiness_prerequisites"
    ]
    ...
    summary = readiness.summarize_durable_executor_authoring_activation_readiness_after_promotion_barrier(
        [contract]
    )
    assert summary["status"] == "passed"
    assert (
        summary["durable_requested_executor_authoring_activation_readiness_after_promotion_barrier_count"]
        == 1
    )
    ...
    future_contract = (
        readiness.build_durable_executor_authoring_activation_readiness_after_promotion_barrier_contract(
            True,
            future_summary,
    ...
    assert future_contract["missing_activation_readiness_prerequisites"] == [
        "separate_durable_executor_authoring_open_after_activation_readiness_contract"
    ]
    ...
    unsafe_summary = readiness.summarize_durable_executor_authoring_activation_readiness_after_promotion_barrier(
        [unsafe_contract]
    )
    ...
    print(
        "BP authoring durable executor authoring activation readiness-after-promotion-barrier contract smoke passed"
    )
    return 0
