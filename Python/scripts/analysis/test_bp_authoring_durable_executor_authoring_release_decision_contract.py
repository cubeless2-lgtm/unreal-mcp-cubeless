#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_release_decision_contract.py."""
...
import bp_authoring_durable_executor_authoring_release_decision_contract as decision  # noqa: E402
import bp_authoring_durable_executor_authoring_release_review_contract as review  # noqa: E402


def build_current_release_review_summary() -> dict:
    return {
        "schema": review.DURABLE_EXECUTOR_AUTHORING_RELEASE_REVIEW_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_release_decision_record(**overrides: object) -> dict:
    record = {
        "schema": decision.DURABLE_EXECUTOR_AUTHORING_RELEASE_DECISION_RECORD_SCHEMA,
...
def main() -> int:
    current_summary = build_current_release_review_summary()
    contract = decision.build_durable_executor_authoring_release_decision_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == decision.DURABLE_EXECUTOR_AUTHORING_RELEASE_DECISION_SCHEMA
    ...
    assert "section_123_release_review_inputs_satisfied" in contract[
        "missing_release_decision_prerequisites"
    ]
    assert "section_123_release_review_record_valid" in contract[
        "missing_release_decision_prerequisites"
    ]
    assert "section_123_allowed_release_review_observed" in contract[
        "missing_release_decision_prerequisites"
    ]
    assert "section_123_no_forbidden_release_review_claims" in contract[
        "missing_release_decision_prerequisites"
    ]
    ...
    assert "separate_durable_authoring_release_promotion_barrier_contract" in contract[
        "missing_release_decision_prerequisites"
    ]
    ...
    summary = decision.summarize_durable_executor_authoring_release_decisions(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_release_decision_count"] == 1
    ...
    future_contract = decision.build_durable_executor_authoring_release_decision_contract(
        True,
        future_summary,
    ...
    assert future_contract["missing_release_decision_prerequisites"] == [
        "separate_durable_authoring_release_promotion_barrier_contract"
    ]
    ...
    unsafe_contract = decision.build_durable_executor_authoring_release_decision_contract(
        True,
        future_summary,
    ...
    unsafe_summary = decision.summarize_durable_executor_authoring_release_decisions(
        [unsafe_contract]
    )
    ...
    code_change_contract = (
        decision.build_durable_executor_authoring_release_decision_contract(
            True,
            future_summary,
    ...
    code_change_summary = (
        decision.summarize_durable_executor_authoring_release_decisions(
            [code_change_contract]
        )
    )
    ...
    print("BP authoring durable executor authoring release decision contract smoke passed")
    return 0
