...
command-execution-after-dispatch-after-command-after-enable-after-open-after-activation-readiness-only
record after the durable executor authoring
command-dispatch-after-command-after-enable-after-open-after-activation-readiness
record is valid. It does not execute live commands, admit evidence, modify
...
DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_AFTER_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_SCHEMA = (
    "section_147_durable_executor_authoring_command_execution_after_dispatch_after_command_after_enable_after_open_after_activation_readiness_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_AFTER_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_RECORD_SCHEMA = (
    "section_147_durable_executor_authoring_command_execution_after_dispatch_after_command_after_enable_after_open_after_activation_readiness_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_AFTER_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_SUMMARY_SCHEMA = (
    "section_147_durable_executor_authoring_command_execution_after_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary_v1"
)
EXPECTED_EXECUTION_SCOPE = "durable_executor_authoring_command_execution_after_dispatch_after_command_after_enable_after_open_after_activation_readiness_only"
...
def build_durable_executor_authoring_command_execution_after_dispatch_after_command_after_enable_after_open_after_activation_readiness_contract(
    requested: bool,
    command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary: Dict[str, Any],
    ...
    dispatch_contract_ready = bool(
        requested
        and command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary.get("status") == "passed"
        and command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary.get("dispatch_contract_defined_count")
        == 1
        and command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary.get("dispatch_record_rejected_count")
        == 0
        and command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary.get("unsafe_dispatch_record_count")
        == 0
        and command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary.get(
            "reported_forbidden_dispatch_count"
        )
        == 0
        and all(
            command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary.get(key) == 0
            for key in PREVIOUS_ZERO_COUNT_KEYS
        )
    )
    dispatch_inputs_satisfied = bool(
        command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary.get("dispatch_inputs_satisfied_count")
        == 1
    )
    dispatch_record_valid = bool(
        command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary.get("dispatch_record_valid_count") == 1
    )
    planned_authoring_commands_present = bool(
        command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary.get(
    ...
    allowed_dispatch_observed = bool(
        command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary.get("allowed_dispatch_observed_count")
        == 1
    )
    no_forbidden_dispatch_claims = bool(
        command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary.get("no_forbidden_dispatch_claims_count")
        == 1
    )
    ...
    if requested:
        if not dispatch_inputs_satisfied:
            missing.append("section_146_dispatch_inputs_satisfied")
        if not dispatch_record_valid:
            missing.append("section_146_dispatch_record_valid")
        if not planned_authoring_commands_present:
            missing.append("section_146_planned_authoring_commands_present")
        if not allowed_authoring_commands_present:
            missing.append("section_146_allowed_authoring_commands_present")
        if not allowed_dispatch_observed:
            missing.append("section_146_allowed_dispatch_observed")
        if not no_forbidden_dispatch_claims:
            missing.append("section_146_no_forbidden_dispatch_claims")
        if not execution_record_present:
            missing.append(
                "durable_authoring_command_execution_after_dispatch_after_command_after_enable_after_open_after_activation_readiness_record_present"
            )
        if not record_schema_matches:
            missing.append(
                "durable_authoring_command_execution_after_dispatch_after_command_after_enable_after_open_after_activation_readiness_record_schema"
            )
        if not execution_scope_matches:
            missing.append(
                "durable_executor_authoring_command_execution_after_dispatch_after_command_after_enable_after_open_after_activation_readiness_only_scope"
            )
        ...
        if not execution_status_passed:
            missing.append(
                "durable_authoring_command_execution_after_dispatch_after_command_after_enable_after_open_after_activation_readiness_status_passed"
            )
        ...
        missing.append("separate_durable_authoring_command_execution_evidence_after_execution_contract")
    execution_record_rejected = bool(
        execution_record_present and not execution_record_valid
    )
    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_command_execution_after_dispatch_after_command_after_enable_after_open_after_activation_readiness",
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_AFTER_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_SCHEMA,
        "requested": requested,
...
def summarize_durable_executor_authoring_command_executions_after_dispatch_after_command_after_enable_after_open_after_activation_readiness(
    contracts: Sequence[Dict[str, Any]]
) -> Dict[str, Any]:
    requested = [contract for contract in contracts if contract.get("requested")]
    rejected_count = _sum_truthy(requested, "execution_record_rejected")
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_AFTER_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_command_execution_after_dispatch_after_command_after_enable_after_open_after_activation_readiness_count": len(
            requested
        ),
