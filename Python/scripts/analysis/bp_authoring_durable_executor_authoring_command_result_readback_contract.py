...
is valid. It admits only no-completion, no-write, and no-save readback evidence;
it does not accept completed, write, dirty-package, save, delete/rename, cleanup,
code, or live command readbacks.
"""
...
DURABLE_EXECUTOR_AUTHORING_COMMAND_RESULT_READBACK_SCHEMA = (
    "section_120_durable_executor_authoring_command_result_readback_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_RESULT_READBACK_RECORD_SCHEMA = (
    "section_120_durable_executor_authoring_command_result_readback_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_RESULT_READBACK_SUMMARY_SCHEMA = (
    "section_120_durable_executor_authoring_command_result_readback_summary_v1"
)
EXPECTED_READBACK_SCOPE = "durable_executor_authoring_command_result_readback_only"
...
def build_durable_executor_authoring_command_result_readback_contract(
    requested: bool,
    ...
    result_contract_ready = bool(
        ...
        and all(result_summary.get(key) == 0 for key in PREVIOUS_ZERO_COUNT_KEYS)
    )
    result_inputs_satisfied = bool(
        result_summary.get("result_inputs_satisfied_count") == 1
    )
    result_record_valid = bool(result_summary.get("result_record_valid_count") == 1)
    allowed_result_observed = bool(
        result_summary.get("allowed_result_observed_count") == 1
    )
    no_forbidden_results = bool(
        result_summary.get("no_forbidden_results_count") == 1
    )
    ...
    if requested:
        if not result_inputs_satisfied:
            missing.append("section_119_result_inputs_satisfied")
        if not result_record_valid:
            missing.append("section_119_result_record_valid")
        if not allowed_result_observed:
            missing.append("section_119_allowed_result_observed")
        if not no_forbidden_results:
            missing.append("section_119_no_forbidden_results")
        ...
        if not readback_scope_matches:
            missing.append("durable_executor_authoring_command_result_readback_only_scope")
    ...
    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_command_result_readback",
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_RESULT_READBACK_SCHEMA,
        "requested": requested,
...
def summarize_durable_executor_authoring_command_result_readbacks(
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_RESULT_READBACK_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_command_result_readback_count": len(
            requested
        ),
    ...
    summary.update({key: _sum_count(requested, key) for key in ALLOWED_READBACK_COUNT_KEYS})
    summary.update({key: _sum_count(requested, key) for key in FORBIDDEN_READBACK_COUNT_KEYS})
    return summary
