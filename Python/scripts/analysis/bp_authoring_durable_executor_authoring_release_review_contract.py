...
authoring final release readiness record is valid. It does not accept release
review execution, release decision start, durable authoring readiness, completion,
asset writes, dirty packages, save, delete/rename, cleanup, code changes, live
probes, live commands, or durable authoring enablement.
"""
...
DURABLE_EXECUTOR_AUTHORING_RELEASE_REVIEW_SCHEMA = (
    "section_123_durable_executor_authoring_release_review_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_RELEASE_REVIEW_RECORD_SCHEMA = (
    "section_123_durable_executor_authoring_release_review_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_RELEASE_REVIEW_SUMMARY_SCHEMA = (
    "section_123_durable_executor_authoring_release_review_summary_v1"
)
EXPECTED_RELEASE_REVIEW_SCOPE = "durable_executor_authoring_release_review_only"
...
def build_durable_executor_authoring_release_review_contract(
    requested: bool,
    ...
    final_release_readiness_contract_ready = bool(
        ...
        and final_release_readiness_summary.get(
        ...
        == 0
        and all(final_release_readiness_summary.get(key) == 0 for key in PREVIOUS_ZERO_COUNT_KEYS)
    )
    final_release_readiness_inputs_satisfied = bool(
        final_release_readiness_summary.get(
    ...
    release_review_scope_matches = bool(
        review_record_present
        and review_record.get("release_review_scope")
    ...
    if requested:
        if not final_release_readiness_inputs_satisfied:
            missing.append("section_122_final_release_readiness_inputs_satisfied")
        if not final_release_readiness_record_valid:
            missing.append("section_122_final_release_readiness_record_valid")
        if not allowed_final_release_readiness_observed:
            missing.append("section_122_allowed_final_release_readiness_observed")
        if not no_forbidden_final_release_readiness_claims:
            missing.append("section_122_no_forbidden_final_release_readiness_claims")
        ...
        if not release_review_scope_matches:
            missing.append("durable_executor_authoring_release_review_only_scope")
        ...
        missing.append("separate_durable_authoring_release_decision_contract")
    release_review_record_rejected = bool(
        review_record_present and not release_review_record_valid
    )
    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_release_review",
        "schema": DURABLE_EXECUTOR_AUTHORING_RELEASE_REVIEW_SCHEMA,
        "requested": requested,
...
def summarize_durable_executor_authoring_release_reviews(
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_RELEASE_REVIEW_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_release_review_count": len(requested),
