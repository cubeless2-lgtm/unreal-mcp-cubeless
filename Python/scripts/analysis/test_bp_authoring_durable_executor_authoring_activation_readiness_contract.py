#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_activation_readiness_contract.py."""
...
import bp_authoring_durable_executor_authoring_activation_readiness_contract as readiness  # noqa: E402
import bp_authoring_durable_executor_authoring_release_promotion_barrier_contract as barrier  # noqa: E402


def build_current_release_promotion_barrier_summary() -> dict:
    return {
        "schema": barrier.DURABLE_EXECUTOR_AUTHORING_RELEASE_PROMOTION_BARRIER_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_activation_readiness_record(**overrides: object) -> dict:
    record = {
        "schema": readiness.DURABLE_EXECUTOR_AUTHORING_ACTIVATION_READINESS_RECORD_SCHEMA,
...
def main() -> int:
    ...
    contract = readiness.build_durable_executor_authoring_activation_readiness_contract(
        True,
        current_summary,
    )
    assert (
        contract["schema"]
        == readiness.DURABLE_EXECUTOR_AUTHORING_ACTIVATION_READINESS_SCHEMA
    )
    ...
    assert "section_125_release_promotion_barrier_inputs_satisfied" in contract[
        "missing_activation_readiness_prerequisites"
    ]
    assert "section_125_release_promotion_barrier_record_valid" in contract[
        "missing_activation_readiness_prerequisites"
    ]
    assert "section_125_allowed_release_promotion_barrier_observed" in contract[
        "missing_activation_readiness_prerequisites"
    ]
    assert "section_125_no_forbidden_release_promotion_barrier_claims" in contract[
        "missing_activation_readiness_prerequisites"
    ]
    ...
    summary = readiness.summarize_durable_executor_authoring_activation_readiness(
        [contract]
    )
    assert summary["status"] == "passed"
    assert (
        summary["durable_requested_executor_authoring_activation_readiness_count"]
        == 1
    )
    ...
    future_contract = (
        readiness.build_durable_executor_authoring_activation_readiness_contract(
            True,
            future_summary,
    ...
    unsafe_summary = readiness.summarize_durable_executor_authoring_activation_readiness(
        [unsafe_contract]
    )
    ...
    print(
        "BP authoring durable executor authoring activation readiness contract smoke passed"
    )
    return 0
