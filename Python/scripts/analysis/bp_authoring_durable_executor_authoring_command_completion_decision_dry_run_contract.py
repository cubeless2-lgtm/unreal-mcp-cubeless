...
This contract defines an offline completion decision dry-run gate after the
Section 166 execution evidence dry-run. It can admit only a completion decision
dry-run record. It does not promote completion decisions, promote execution
evidence, open command paths, dispatch live commands, execute live commands,
...
DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_DRY_RUN_SCHEMA = (
    "section_167_durable_executor_authoring_command_completion_decision_dry_run_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_DRY_RUN_RECORD_SCHEMA = (
    "section_167_durable_executor_authoring_command_completion_decision_dry_run_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_DRY_RUN_SUMMARY_SCHEMA = (
    "section_167_durable_executor_authoring_command_completion_decision_dry_run_summary_v1"
)
EXPECTED_COMPLETION_DECISION_SCOPE = (
    "durable_executor_authoring_command_completion_decision_dry_run_only"
)

ALLOWED_COMPLETION_DECISION_DRY_RUN_OPERATIONS = (
    "validate_completion_decision_envelope",
    "evaluate_completion_decision_readiness",
    "completion_decision_dry_run_only",
)
...
OUTPUT_ACTION_KEYS = (
    "completion_decision_dry_run_started",
    "completion_decision_dry_run_accepted",
...
def _chain_flags(section_166_execution_evidence_summary: Dict[str, Any]) -> Dict[str, bool]:
    return {
        output_key: section_166_execution_evidence_summary.get(summary_key) == 1
        for output_key, summary_key, _missing_key in CHAIN_INPUTS
    }


def build_durable_executor_authoring_command_completion_decision_dry_run_contract(
    requested: bool,
    section_166_command_execution_evidence_dry_run_summary: Dict[str, Any],
    completion_decision_record: Dict[str, Any] | None = None,
) -> Dict[str, Any]:
    completion_decision_record = completion_decision_record or {}
    completion_decision_record_present = bool(completion_decision_record)
    section_166_execution_evidence_contract_ready = bool(
        requested
        and section_166_command_execution_evidence_dry_run_summary.get("status")
        == "passed"
        and section_166_command_execution_evidence_dry_run_summary.get(
            "execution_evidence_contract_defined_count"
        )
        == 1
        and section_166_command_execution_evidence_dry_run_summary.get(
            "execution_evidence_dry_run_record_rejected_count"
        )
        == 0
        and section_166_command_execution_evidence_dry_run_summary.get(
        ...
        == 0
        and all(
            section_166_command_execution_evidence_dry_run_summary.get(key) == 0
            for key in SECTION_166_ZERO_COUNT_KEYS
        )
    )
    chain_flags = _chain_flags(
        section_166_command_execution_evidence_dry_run_summary
    )
    execution_evidence_chain_satisfied = all(chain_flags.values())
    completion_decision_contract_defined = bool(
        requested and section_166_execution_evidence_contract_ready
    )

    record_schema_matches = bool(
        completion_decision_record_present
        and completion_decision_record.get("schema")
        == DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_DRY_RUN_RECORD_SCHEMA
    )
    completion_decision_scope_matches = bool(
        completion_decision_record_present
        and completion_decision_record.get("completion_decision_scope")
        == EXPECTED_COMPLETION_DECISION_SCOPE
    )
    dry_run_only = bool(
        completion_decision_record_present
        and completion_decision_record.get("dry_run_only") is True
    )
    ...
    operator_reconfirmed_no_live_dispatch = bool(
        completion_decision_record_present
        and completion_decision_record.get("operator_reconfirmed_no_live_dispatch")
        is True
    )
    operator_reconfirmed_no_live_execution = bool(
        completion_decision_record_present
        and completion_decision_record.get("operator_reconfirmed_no_live_execution")
        is True
    )
    operator_reconfirmed_no_write_execution = bool(
        completion_decision_record_present
        and completion_decision_record.get("operator_reconfirmed_no_write_execution")
        is True
    )
    ...
    requested_command = completion_decision_record.get("requested_command", "")
    completion_decision_operation = completion_decision_record.get(
        "completion_decision_operation", ""
    )
    target_asset = completion_decision_record.get("target_asset", "")
    ...
    completion_decision_operation_allowed = bool(
        completion_decision_operation
        in ALLOWED_COMPLETION_DECISION_DRY_RUN_OPERATIONS
    )
    completion_decision_target_declared = bool(
        isinstance(target_asset, str) and target_asset.startswith("/Game/")
    )
    execution_evidence_admission_proof_matches = all(
        _proof_flag(
            completion_decision_record,
            "execution_evidence_admission_proof",
            key,
        )
    ...
    release_boundary_proof_safe = bool(
        completion_decision_record_present
        and completion_decision_record.get("release_boundary_proof", {}).get(
            "durable_authoring_enabled"
        )
        is False
        and completion_decision_record.get("release_boundary_proof", {}).get(
    ...
    completion_decision_dry_run_record_valid = bool(
        completion_decision_contract_defined
        and execution_evidence_chain_satisfied
        and record_schema_matches
        and completion_decision_scope_matches
        and dry_run_only
        ...
        and completion_decision_operation_allowed
        and completion_decision_target_declared
        and execution_evidence_admission_proof_matches
        and release_boundary_proof_safe
        and unsafe_completion_decision_record_count == 0
    )
    completion_decision_dry_run_record_rejected = bool(
        completion_decision_record_present
        and not completion_decision_dry_run_record_valid
    )
    completion_decision_dry_run_admissible = bool(
        completion_decision_dry_run_record_valid
    )

    missing: list[str] = []
    if requested:
        if not section_166_execution_evidence_contract_ready:
            missing.append(
                "section_166_command_execution_evidence_dry_run_contract_ready"
            )
        for output_key, _summary_key, missing_key in CHAIN_INPUTS:
            if not chain_flags[output_key]:
                missing.append(missing_key)
        if not completion_decision_record_present:
            missing.append("command_completion_decision_dry_run_record_present")
        if not record_schema_matches:
            missing.append("command_completion_decision_dry_run_record_schema")
        if not completion_decision_scope_matches:
            missing.append("command_completion_decision_dry_run_only_scope")
        if not dry_run_only:
            missing.append("completion_decision_dry_run_only")
        if not completion_decision_status_passed:
            missing.append("command_completion_decision_dry_run_status_passed")
        ...
        if not completion_decision_operation_allowed:
            missing.append("allowed_completion_decision_dry_run_operation")
        if not completion_decision_target_declared:
            missing.append("completion_decision_target_declared")
        if not execution_evidence_admission_proof_matches:
            missing.append("execution_evidence_admission_proof_matches")
        if not release_boundary_proof_safe:
            missing.append("release_boundary_proof_safe")

    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_command_completion_decision_dry_run",
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_DRY_RUN_SCHEMA,
        "requested": requested,
        "completion_decision_contract_defined": (
            completion_decision_contract_defined
        ),
        ...
        "expected_completion_decision_scope": (
            EXPECTED_COMPLETION_DECISION_SCOPE if requested else ""
        ),
        "allowed_completion_decision_dry_run_operations": (
            list(ALLOWED_COMPLETION_DECISION_DRY_RUN_OPERATIONS)
            if requested
            else []
        ),
        "allowed_requested_commands": (
        ...
        "section_166_execution_evidence_contract_ready": (
            section_166_execution_evidence_contract_ready
        ),
        "execution_evidence_chain_satisfied": execution_evidence_chain_satisfied,
        "completion_decision_dry_run_record_present": (
            completion_decision_record_present
        ),
        "record_schema_matches": record_schema_matches,
        "completion_decision_scope_matches": completion_decision_scope_matches,
        ...
        "completion_decision_operation_allowed": (
            completion_decision_operation_allowed
        ),
        "completion_decision_target_declared": completion_decision_target_declared,
        "execution_evidence_admission_proof_matches": (
            execution_evidence_admission_proof_matches
        ),
        ...
        "completion_decision_dry_run_record_rejected": (
            completion_decision_dry_run_record_rejected
        ),
        "completion_decision_dry_run_admissible": (
            completion_decision_dry_run_admissible
        ...
        "missing_completion_decision_dry_run_prerequisites": missing,
        "missing_completion_decision_dry_run_prerequisite_count": len(missing),
    }
    contract.update(chain_flags)
    contract.update({key: False for key in OUTPUT_ACTION_KEYS})
    contract["completion_decision_dry_run_admissible"] = (
        completion_decision_dry_run_admissible
    )
    return contract


def summarize_durable_executor_authoring_command_completion_decision_dry_runs(
    contracts: Sequence[Dict[str, Any]]
) -> Dict[str, Any]:
    requested = [contract for contract in contracts if contract.get("requested")]
    rejected_count = _sum_truthy(
        requested, "completion_decision_dry_run_record_rejected"
    )
    unsafe_count = _sum_count(
        requested, "unsafe_completion_decision_record_count"
    )
    ...
    if requested:
        status = (
            ...
            and all(
                ...
                if key != "completion_decision_dry_run_admissible"
            )
            else "failed"
        )
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_DECISION_DRY_RUN_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_command_completion_decision_dry_run_count": len(
            requested
        ),
        "completion_decision_contract_defined_count": _sum_truthy(
            requested, "completion_decision_contract_defined"
        ),
        "section_166_execution_evidence_contract_ready_count": _sum_truthy(
            requested, "section_166_execution_evidence_contract_ready"
        ),
    }
    summary.update(
        {
    ...
    summary.update(
        {
            "execution_evidence_chain_satisfied_count": _sum_truthy(
                requested, "execution_evidence_chain_satisfied"
            ),
            "completion_decision_dry_run_record_present_count": _sum_truthy(
                requested, "completion_decision_dry_run_record_present"
            ),
            "record_schema_matches_count": _sum_truthy(
                requested, "record_schema_matches"
            ),
            "completion_decision_scope_matches_count": _sum_truthy(
                requested, "completion_decision_scope_matches"
            ),
            ...
            "completion_decision_operation_allowed_count": _sum_truthy(
                requested, "completion_decision_operation_allowed"
            ),
            "completion_decision_target_declared_count": _sum_truthy(
                requested, "completion_decision_target_declared"
            ),
            "execution_evidence_admission_proof_matches_count": _sum_truthy(
                requested, "execution_evidence_admission_proof_matches"
            ),
            "release_boundary_proof_safe_count": _sum_truthy(
                requested, "release_boundary_proof_safe"
            ),
            "completion_decision_dry_run_record_valid_count": _sum_truthy(
                requested, "completion_decision_dry_run_record_valid"
            ),
            "completion_decision_dry_run_record_rejected_count": rejected_count,
            "completion_decision_dry_run_admissible_count": _sum_truthy(
                requested, "completion_decision_dry_run_admissible"
            ),
            "unsafe_completion_decision_record_count": unsafe_count,
            "missing_completion_decision_dry_run_prerequisite_count": _sum_count(
                requested,
                "missing_completion_decision_dry_run_prerequisite_count",
            ),
        }
    )
    summary.update(
