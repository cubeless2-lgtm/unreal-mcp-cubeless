#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_implementation_review_contract.py."""
...
def build_current_final_release_readiness_summary() -> dict:
    return {
        "schema": readiness.CANARY_DURABLE_AUTHORING_FINAL_RELEASE_READINESS_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_review_record(**overrides: object) -> dict:
    record = {
        "schema": review.DURABLE_EXECUTOR_IMPLEMENTATION_REVIEW_RECORD_SCHEMA,
        "implementation_review_scope": review.EXPECTED_IMPLEMENTATION_REVIEW_SCOPE,
        "explicit_durable_executor_implementation_review_authorized": True,
        "status": "passed",
        ...
        "reported_contract_chain_review_count": 1,
        "reported_no_write_review_count": 1,
        "reported_no_save_review_count": 1,
        "reported_executor_surface_review_count": 1,
        "reported_implementation_review_started_count": 0,
...
def main() -> int:
    ...
    contract = review.build_durable_executor_implementation_review_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == review.DURABLE_EXECUTOR_IMPLEMENTATION_REVIEW_SCHEMA
    assert contract["requested"] is True
    assert contract["implementation_review_contract_defined"] is True
    ...
    assert contract["implementation_review_record_present"] is False
    assert contract["implementation_review_record_valid"] is False
    assert contract["implementation_review_record_rejected"] is False
    assert contract["unsafe_implementation_review_record_count"] == 0
    assert contract["missing_implementation_review_prerequisite_count"] == 14
    assert "section_94_final_release_readiness_inputs_satisfied" in contract[
        "missing_implementation_review_prerequisites"
    ]
    assert "section_94_final_release_readiness_record_valid" in contract[
        "missing_implementation_review_prerequisites"
    ]
    assert "section_94_allowed_final_release_readiness_observed" in contract[
        "missing_implementation_review_prerequisites"
    ]
    assert "section_94_no_forbidden_final_release_readiness_claims" in contract[
        "missing_implementation_review_prerequisites"
    ]
    assert "durable_executor_implementation_review_record_present" in contract[
        "missing_implementation_review_prerequisites"
    ]
    assert "explicit_durable_mvp_request_reconfirmed" in contract[
        "missing_implementation_review_prerequisites"
    ]
    assert "separate_durable_executor_implementation_plan_contract" in contract[
        "missing_implementation_review_prerequisites"
    ]
    ...
    summary = review.summarize_durable_executor_implementation_reviews([contract])
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_implementation_review_count"] == 1
    assert summary["implementation_review_contract_defined_count"] == 1
    assert summary["final_release_readiness_contract_ready_count"] == 1
    assert summary["implementation_review_record_valid_count"] == 0
    assert summary["implementation_review_record_rejected_count"] == 0
    assert summary["unsafe_implementation_review_record_count"] == 0
    assert summary["missing_implementation_review_prerequisite_count"] == 14
    ...
    future_contract = review.build_durable_executor_implementation_review_contract(
        True,
        future_summary,
        build_review_record(),
    )
    assert future_contract["implementation_review_record_valid"] is True
    assert future_contract["missing_implementation_review_prerequisite_count"] == 1
    assert future_contract["missing_implementation_review_prerequisites"] == [
        "separate_durable_executor_implementation_plan_contract"
    ]
    assert future_contract["reported_allowed_implementation_review_count"] == 4
    assert future_contract["reported_forbidden_implementation_review_count"] == 0
    ...
    unsafe_contract = review.build_durable_executor_implementation_review_contract(
        True,
        future_summary,
        build_review_record(code_change_performed=True),
    )
    assert unsafe_contract["implementation_review_record_valid"] is False
    assert unsafe_contract["implementation_review_record_rejected"] is True
    assert unsafe_contract["unsafe_implementation_review_record_count"] == 1
    unsafe_summary = review.summarize_durable_executor_implementation_reviews([unsafe_contract])
    assert unsafe_summary["status"] == "failed"
    assert unsafe_summary["implementation_review_record_rejected_count"] == 1
    assert unsafe_summary["unsafe_implementation_review_record_count"] == 1
    ...
    print("BP authoring durable executor implementation review contract smoke passed")
    return 0
