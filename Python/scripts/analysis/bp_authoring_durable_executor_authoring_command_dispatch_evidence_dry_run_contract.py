...
This contract defines an offline dispatch evidence dry-run gate after the
Section 163 command dispatch dry-run. It can admit only a dispatch evidence
dry-run record. It does not promote durable evidence, promote dispatch
envelopes, open command paths, dispatch live commands, execute commands,
...
DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_EVIDENCE_DRY_RUN_SCHEMA = (
    "section_164_durable_executor_authoring_command_dispatch_evidence_dry_run_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_EVIDENCE_DRY_RUN_RECORD_SCHEMA = (
    "section_164_durable_executor_authoring_command_dispatch_evidence_dry_run_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_EVIDENCE_DRY_RUN_SUMMARY_SCHEMA = (
    "section_164_durable_executor_authoring_command_dispatch_evidence_dry_run_summary_v1"
)
EXPECTED_EVIDENCE_SCOPE = (
    "durable_executor_authoring_command_dispatch_evidence_dry_run_only"
)

ALLOWED_DISPATCH_EVIDENCE_DRY_RUN_OPERATIONS = (
    "validate_dispatch_evidence_envelope",
    "evaluate_dispatch_evidence_readiness",
    "dispatch_evidence_dry_run_only",
)
...
OUTPUT_ACTION_KEYS = (
    "dispatch_evidence_dry_run_started",
    "dispatch_evidence_dry_run_accepted",
...
def build_durable_executor_authoring_command_dispatch_evidence_dry_run_contract(
    requested: bool,
    section_163_command_dispatch_dry_run_summary: Dict[str, Any],
    evidence_record: Dict[str, Any] | None = None,
) -> Dict[str, Any]:
    evidence_record = evidence_record or {}
    evidence_record_present = bool(evidence_record)
    section_163_dispatch_contract_ready = bool(
        requested
        and section_163_command_dispatch_dry_run_summary.get("status") == "passed"
        and section_163_command_dispatch_dry_run_summary.get(
            "dispatch_contract_defined_count"
        )
        == 1
        and section_163_command_dispatch_dry_run_summary.get(
            "dispatch_dry_run_record_rejected_count"
        )
        == 0
        and section_163_command_dispatch_dry_run_summary.get(
        ...
        == 0
        and all(
            section_163_command_dispatch_dry_run_summary.get(key) == 0
            for key in SECTION_163_ZERO_COUNT_KEYS
        )
    )
    open_activation_promotion_readiness_chain_satisfied = bool(
        section_163_command_dispatch_dry_run_summary.get(
    ...
    dispatch_dry_run_admissible = bool(
        section_163_command_dispatch_dry_run_summary.get(
            "dispatch_dry_run_admissible_count"
        )
        == 1
    )
    dispatch_chain_satisfied = bool(
        ...
        and dispatch_dry_run_record_valid
        and dispatch_dry_run_admissible
    )
    dispatch_evidence_contract_defined = bool(
        requested and section_163_dispatch_contract_ready
    )
    ...
    dry_run_only = bool(
        evidence_record_present and evidence_record.get("dry_run_only") is True
    )
    ...
    operator_reconfirmed_no_live_dispatch = bool(
        evidence_record_present
        and evidence_record.get("operator_reconfirmed_no_live_dispatch") is True
    )
    operator_reconfirmed_no_live_execution = bool(
        evidence_record_present
        and evidence_record.get("operator_reconfirmed_no_live_execution") is True
    )
    operator_reconfirmed_no_write_execution = bool(
        evidence_record_present
        and evidence_record.get("operator_reconfirmed_no_write_execution") is True
    )
    ...
    requested_command = evidence_record.get("requested_command", "")
    evidence_operation = evidence_record.get("evidence_operation", "")
    target_asset = evidence_record.get("target_asset", "")
    ...
    evidence_operation_allowed = bool(
        evidence_operation in ALLOWED_DISPATCH_EVIDENCE_DRY_RUN_OPERATIONS
    )
    dispatch_evidence_target_declared = bool(
        isinstance(target_asset, str) and target_asset.startswith("/Game/")
    )
    dispatch_admission_proof_matches = bool(
        _proof_flag(
            evidence_record,
            "dispatch_admission_proof",
            "open_activation_promotion_readiness_chain_satisfied",
        )
        and _proof_flag(
            evidence_record,
    ...
    release_boundary_proof_safe = bool(
        evidence_record_present
        and evidence_record.get("release_boundary_proof", {}).get(
            "durable_authoring_enabled"
        )
        is False
        and evidence_record.get("release_boundary_proof", {}).get(
    ...
    unsafe_evidence_record_count = sum(
        int(_attempted(evidence_record.get(key)))
        for key in UNSAFE_EVIDENCE_RECORD_ACTION_KEYS
    )
    dispatch_evidence_dry_run_record_valid = bool(
        dispatch_evidence_contract_defined
        and dispatch_chain_satisfied
        ...
        and evidence_operation_allowed
        and dispatch_evidence_target_declared
        and dispatch_admission_proof_matches
        and release_boundary_proof_safe
        and unsafe_evidence_record_count == 0
    )
    dispatch_evidence_dry_run_record_rejected = bool(
        evidence_record_present and not dispatch_evidence_dry_run_record_valid
    )
    dispatch_evidence_dry_run_admissible = bool(
        dispatch_evidence_dry_run_record_valid
    )

    missing: list[str] = []
    if requested:
        if not section_163_dispatch_contract_ready:
            missing.append("section_163_command_dispatch_dry_run_contract_ready")
        if not open_activation_promotion_readiness_chain_satisfied:
            missing.append("section_163_open_activation_promotion_readiness_chain_satisfied")
        if not authoring_enable_chain_satisfied:
            missing.append("section_163_authoring_enable_chain_satisfied")
        if not durable_release_readiness_chain_reconfirmed:
            missing.append("section_163_durable_release_readiness_chain_reconfirmed")
        if not authoring_command_inputs_satisfied:
            missing.append("section_163_authoring_command_inputs_satisfied")
        if not authoring_command_record_valid:
            missing.append("section_163_authoring_command_record_valid")
        if not dry_run_route_record_valid:
            missing.append("section_163_dry_run_route_record_valid")
        if not dry_run_route_admissible:
            missing.append("section_163_dry_run_route_admissible")
        if not dispatch_dry_run_record_valid:
            missing.append("section_163_dispatch_dry_run_record_valid")
        if not dispatch_dry_run_admissible:
            missing.append("section_163_dispatch_dry_run_admissible")
        if not evidence_record_present:
            missing.append("command_dispatch_evidence_dry_run_record_present")
        if not record_schema_matches:
            missing.append("command_dispatch_evidence_dry_run_record_schema")
        if not evidence_scope_matches:
            missing.append("command_dispatch_evidence_dry_run_only_scope")
        if not dry_run_only:
            missing.append("dispatch_evidence_dry_run_only")
        if not evidence_status_passed:
            missing.append("command_dispatch_evidence_dry_run_status_passed")
        ...
        if not evidence_operation_allowed:
            missing.append("allowed_dispatch_evidence_dry_run_operation")
        if not dispatch_evidence_target_declared:
            missing.append("dispatch_evidence_target_declared")
        if not dispatch_admission_proof_matches:
            missing.append("dispatch_admission_proof_matches")
        if not release_boundary_proof_safe:
            missing.append("release_boundary_proof_safe")

    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_command_dispatch_evidence_dry_run",
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_EVIDENCE_DRY_RUN_SCHEMA,
        "requested": requested,
        "dispatch_evidence_contract_defined": dispatch_evidence_contract_defined,
        ...
        "allowed_dispatch_evidence_dry_run_operations": (
            list(ALLOWED_DISPATCH_EVIDENCE_DRY_RUN_OPERATIONS) if requested else []
        ),
        "allowed_requested_commands": (
        ...
        "section_163_dispatch_contract_ready": section_163_dispatch_contract_ready,
        "open_activation_promotion_readiness_chain_satisfied": (
            open_activation_promotion_readiness_chain_satisfied
        ),
        ...
        "dispatch_chain_satisfied": dispatch_chain_satisfied,
        "dispatch_evidence_dry_run_record_present": evidence_record_present,
        ...
        "evidence_operation_allowed": evidence_operation_allowed,
        "dispatch_evidence_target_declared": dispatch_evidence_target_declared,
        "dispatch_admission_proof_matches": dispatch_admission_proof_matches,
        "release_boundary_proof_safe": release_boundary_proof_safe,
        "dispatch_evidence_dry_run_record_valid": (
            dispatch_evidence_dry_run_record_valid
        ),
        "dispatch_evidence_dry_run_record_rejected": (
            dispatch_evidence_dry_run_record_rejected
        ),
        "dispatch_evidence_dry_run_admissible": dispatch_evidence_dry_run_admissible,
        "unsafe_evidence_record_count": unsafe_evidence_record_count,
        "missing_dispatch_evidence_dry_run_prerequisites": missing,
        "missing_dispatch_evidence_dry_run_prerequisite_count": len(missing),
    }
    contract.update({key: False for key in OUTPUT_ACTION_KEYS})
    contract["dispatch_evidence_dry_run_admissible"] = (
        dispatch_evidence_dry_run_admissible
    )
    return contract


def summarize_durable_executor_authoring_command_dispatch_evidence_dry_runs(
    contracts: Sequence[Dict[str, Any]]
) -> Dict[str, Any]:
    requested = [contract for contract in contracts if contract.get("requested")]
    rejected_count = _sum_truthy(
        requested, "dispatch_evidence_dry_run_record_rejected"
    )
    ...
    if requested:
        status = (
            "passed"
            if _sum_truthy(requested, "dispatch_evidence_contract_defined")
            == len(requested)
            ...
            and all(
                ...
                if key != "dispatch_evidence_dry_run_admissible"
            )
            else "failed"
        )
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_EVIDENCE_DRY_RUN_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_command_dispatch_evidence_dry_run_count": len(
            requested
        ),
        "dispatch_evidence_contract_defined_count": _sum_truthy(
            requested, "dispatch_evidence_contract_defined"
        ),
        "section_163_dispatch_contract_ready_count": _sum_truthy(
            requested, "section_163_dispatch_contract_ready"
        ),
        ...
        "dispatch_chain_satisfied_count": _sum_truthy(
            requested, "dispatch_chain_satisfied"
        ),
        "dispatch_evidence_dry_run_record_present_count": _sum_truthy(
            requested, "dispatch_evidence_dry_run_record_present"
        ),
        ...
        "evidence_operation_allowed_count": _sum_truthy(
            requested, "evidence_operation_allowed"
        ),
        "dispatch_evidence_target_declared_count": _sum_truthy(
            requested, "dispatch_evidence_target_declared"
        ),
        "dispatch_admission_proof_matches_count": _sum_truthy(
            requested, "dispatch_admission_proof_matches"
        ),
        "release_boundary_proof_safe_count": _sum_truthy(
            requested, "release_boundary_proof_safe"
        ),
        "dispatch_evidence_dry_run_record_valid_count": _sum_truthy(
            requested, "dispatch_evidence_dry_run_record_valid"
        ),
        "dispatch_evidence_dry_run_record_rejected_count": rejected_count,
        "dispatch_evidence_dry_run_admissible_count": _sum_truthy(
            requested, "dispatch_evidence_dry_run_admissible"
        ),
        "unsafe_evidence_record_count": unsafe_count,
        "missing_dispatch_evidence_dry_run_prerequisite_count": _sum_count(
            requested, "missing_dispatch_evidence_dry_run_prerequisite_count"
        ),
    }
    summary.update(
        {
