#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_contract.py."""
...
import bp_authoring_durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_contract as dispatch  # noqa: E402


def build_current_authoring_command_after_enable_after_open_after_activation_readiness_summary() -> dict:
    return {
        "schema": command.DURABLE_EXECUTOR_AUTHORING_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_dispatch_record(**overrides: object) -> dict:
    record = {
        "schema": dispatch.DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_RECORD_SCHEMA,
        ...
        "durable_authoring_command_dispatch_started": False,
        "durable_authoring_command_dispatch_accepted": False,
        "durable_authoring_command_dispatch_allowed": False,
        "durable_authoring_command_dispatched": False,
        "durable_authoring_command_execution_contract_started": False,
        ...
        "reported_dispatch_gate_count": 1,
        "reported_authoring_command_revalidated_count": 1,
        "reported_no_live_dispatch_performed_count": 1,
        "reported_no_execution_authorized_count": 1,
        "reported_no_asset_write_dispatch_count": 1,
        "reported_no_save_delete_rename_dispatch_count": 1,
...
def main() -> int:
    current_summary = build_current_authoring_command_after_enable_after_open_after_activation_readiness_summary()
    contract = dispatch.build_durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_contract(
        True,
        current_summary,
    )
    assert (
        contract["schema"]
        == dispatch.DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_SCHEMA
    )
    ...
    assert contract["missing_dispatch_prerequisite_count"] == 14
    assert "section_145_authoring_command_inputs_satisfied" in contract[
        "missing_dispatch_prerequisites"
    ]
    assert "section_145_authoring_command_record_valid" in contract[
        "missing_dispatch_prerequisites"
    ]
    assert "section_145_planned_authoring_commands_present" in contract[
        "missing_dispatch_prerequisites"
    ]
    assert "section_145_allowed_authoring_commands_present" in contract[
        "missing_dispatch_prerequisites"
    ]
    assert "durable_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_record_present" in contract[
        "missing_dispatch_prerequisites"
    ]
    assert "explicit_durable_mvp_request_reconfirmed" in contract[
        "missing_dispatch_prerequisites"
    ]
    assert "separate_durable_authoring_command_execution_after_dispatch_contract" in contract[
        "missing_dispatch_prerequisites"
    ]
    assert contract["durable_authoring_command_dispatch_started"] is False
    assert contract["durable_authoring_command_dispatch_accepted"] is False
    ...
    assert contract["durable_authoring_command_execution_contract_started"] is False
    ...
    summary = dispatch.summarize_durable_executor_authoring_command_dispatches_after_command_after_enable_after_open_after_activation_readiness(
        [contract]
    )
    assert summary["status"] == "passed"
    assert (
        summary[
            "durable_requested_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_count"
        ]
        == 1
    )
    assert summary["dispatch_contract_defined_count"] == 1
    assert summary["authoring_command_contract_ready_count"] == 1
    assert summary["dispatch_record_valid_count"] == 0
    assert summary["dispatch_record_rejected_count"] == 0
    assert summary["unsafe_dispatch_record_count"] == 0
    assert summary["missing_dispatch_prerequisite_count"] == 14
    assert summary["reported_allowed_dispatch_count"] == 0
    assert summary["reported_forbidden_dispatch_count"] == 0
    assert summary["durable_authoring_command_dispatch_started_count"] == 0
    assert summary["durable_authoring_command_dispatch_accepted_count"] == 0
    assert summary["durable_authoring_command_dispatch_allowed_count"] == 0
    assert summary["durable_authoring_command_dispatched_count"] == 0
    assert summary["durable_authoring_command_execution_contract_started_count"] == 0
    ...
    future_contract = (
        dispatch.build_durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_contract(
            True,
            future_summary,
    ...
    assert future_contract["missing_dispatch_prerequisites"] == [
        "separate_durable_authoring_command_execution_after_dispatch_contract"
    ]
    assert future_contract["reported_allowed_dispatch_count"] == 6
    assert future_contract["reported_forbidden_dispatch_count"] == 0
    ...
    assert future_contract["durable_authoring_command_execution_contract_started"] is False
    ...
    unsafe_summary = dispatch.summarize_durable_executor_authoring_command_dispatches_after_command_after_enable_after_open_after_activation_readiness(
        [unsafe_contract]
    )
    ...
    forbidden_contract = (
        dispatch.build_durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_contract(
            True,
            future_summary,
            build_dispatch_record(reported_live_dispatch_count=1),
        )
    )
    assert forbidden_contract["dispatch_record_valid"] is False
    assert forbidden_contract["dispatch_record_rejected"] is True
    assert forbidden_contract["reported_forbidden_dispatch_count"] == 1
    assert forbidden_contract["unsafe_dispatch_record_count"] == 1

    print(
        "BP authoring durable executor authoring command dispatch-after-command-after-enable-after-open-after-activation-readiness contract smoke passed"
    )
    return 0
