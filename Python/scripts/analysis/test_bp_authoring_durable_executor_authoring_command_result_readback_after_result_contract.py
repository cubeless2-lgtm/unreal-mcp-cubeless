#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_command_result_readback_after_result_contract.py."""
...
def build_current_result_after_application_summary() -> dict:
    return {
        "schema": result.DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_RESULT_AFTER_APPLICATION_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_readback_record(**overrides: object) -> dict:
    record = {
        "schema": readback.DURABLE_EXECUTOR_AUTHORING_COMMAND_RESULT_READBACK_AFTER_RESULT_RECORD_SCHEMA,
...
def main() -> int:
    current_summary = build_current_result_after_application_summary()
    contract = readback.build_durable_executor_authoring_command_result_readback_after_result_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == readback.DURABLE_EXECUTOR_AUTHORING_COMMAND_RESULT_READBACK_AFTER_RESULT_SCHEMA
    ...
    assert "section_135_result_inputs_satisfied" in contract[
        "missing_readback_prerequisites"
    ]
    assert "section_135_result_record_valid" in contract[
        "missing_readback_prerequisites"
    ]
    assert "section_135_allowed_result_observed" in contract[
        "missing_readback_prerequisites"
    ]
    assert "section_135_no_forbidden_results" in contract[
        "missing_readback_prerequisites"
    ]
    ...
    summary = readback.summarize_durable_executor_authoring_command_result_readbacks_after_result(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_command_result_readback_after_result_count"] == 1
    ...
    future_contract = readback.build_durable_executor_authoring_command_result_readback_after_result_contract(
        True,
        future_summary,
    ...
    unsafe_contract = readback.build_durable_executor_authoring_command_result_readback_after_result_contract(
        True,
        future_summary,
    ...
    unsafe_summary = readback.summarize_durable_executor_authoring_command_result_readbacks_after_result(
        [unsafe_contract]
    )
    ...
    print(
        "BP authoring durable executor authoring command result readback-after-result contract smoke passed"
    )
    return 0
