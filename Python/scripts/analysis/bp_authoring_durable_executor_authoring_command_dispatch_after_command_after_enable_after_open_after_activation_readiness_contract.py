...
command-dispatch-after-command-after-enable-after-open-after-activation-readiness-only
record after the durable executor authoring
command-after-enable-after-open-after-activation-readiness record is valid. It
does not dispatch or execute live commands, write assets, dirty packages, save,
...
DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_SCHEMA = (
    "section_146_durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_RECORD_SCHEMA = (
    "section_146_durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_SUMMARY_SCHEMA = (
    "section_146_durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_summary_v1"
)
EXPECTED_DISPATCH_SCOPE = "durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_only"
...
def build_durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_contract(
    requested: bool,
    authoring_command_after_enable_after_open_after_activation_readiness_summary: Dict[str, Any],
    ...
    authoring_command_contract_ready = bool(
        requested
        and authoring_command_after_enable_after_open_after_activation_readiness_summary.get("status") == "passed"
        and authoring_command_after_enable_after_open_after_activation_readiness_summary.get("authoring_command_contract_defined_count") == 1
        and authoring_command_after_enable_after_open_after_activation_readiness_summary.get("authoring_command_record_rejected_count") == 0
        and authoring_command_after_enable_after_open_after_activation_readiness_summary.get("unsafe_authoring_command_record_count") == 0
        and authoring_command_after_enable_after_open_after_activation_readiness_summary.get("forbidden_authoring_command_count") == 0
        and authoring_command_after_enable_after_open_after_activation_readiness_summary.get("unknown_authoring_command_count") == 0
        and all(
            authoring_command_after_enable_after_open_after_activation_readiness_summary.get(key) == 0 for key in PREVIOUS_ZERO_COUNT_KEYS
        )
    )
    authoring_command_inputs_satisfied = bool(
        authoring_command_after_enable_after_open_after_activation_readiness_summary.get("authoring_command_inputs_satisfied_count") == 1
    )
    authoring_command_record_valid = bool(
        authoring_command_after_enable_after_open_after_activation_readiness_summary.get("authoring_command_record_valid_count") == 1
    )
    planned_authoring_commands_present = bool(
        authoring_command_after_enable_after_open_after_activation_readiness_summary.get("planned_authoring_command_count", 0) > 0
    )
    allowed_authoring_commands_present = bool(
        authoring_command_after_enable_after_open_after_activation_readiness_summary.get("allowed_authoring_command_count", 0) > 0
    )
    ...
    if requested:
        if not authoring_command_inputs_satisfied:
            missing.append("section_145_authoring_command_inputs_satisfied")
        if not authoring_command_record_valid:
            missing.append("section_145_authoring_command_record_valid")
        if not planned_authoring_commands_present:
            missing.append("section_145_planned_authoring_commands_present")
        if not allowed_authoring_commands_present:
            missing.append("section_145_allowed_authoring_commands_present")
        if not dispatch_record_present:
            missing.append("durable_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_record_present")
        if not record_schema_matches:
            missing.append("durable_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_record_schema")
        if not dispatch_scope_matches:
            missing.append(
                "durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_only_scope"
            )
        ...
        if not dispatch_status_passed:
            missing.append(
                "durable_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_status_passed"
            )
        ...
        missing.append("separate_durable_authoring_command_execution_after_dispatch_contract")
    dispatch_record_rejected = bool(dispatch_record_present and not dispatch_record_valid)
    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness",
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_SCHEMA,
        "requested": requested,
...
def summarize_durable_executor_authoring_command_dispatches_after_command_after_enable_after_open_after_activation_readiness(
    contracts: Sequence[Dict[str, Any]]
) -> Dict[str, Any]:
    requested = [contract for contract in contracts if contract.get("requested")]
    rejected_count = _sum_truthy(requested, "dispatch_record_rejected")
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_AFTER_COMMAND_AFTER_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_command_dispatch_after_command_after_enable_after_open_after_activation_readiness_count": len(
            requested
        ),
