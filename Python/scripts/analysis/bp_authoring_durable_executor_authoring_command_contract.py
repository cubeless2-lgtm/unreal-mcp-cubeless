...
This contract validates a future durable-executor-authoring-command-only record
after the durable executor authoring enable record is valid. It does not
dispatch or execute live commands, modify assets, save, delete/rename, or
cleanup.
"""
...
DURABLE_EXECUTOR_AUTHORING_COMMAND_SCHEMA = (
    "section_113_durable_executor_authoring_command_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_RECORD_SCHEMA = (
    "section_113_durable_executor_authoring_command_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_SUMMARY_SCHEMA = (
    "section_113_durable_executor_authoring_command_summary_v1"
)
EXPECTED_COMMAND_SCOPE = "durable_executor_authoring_command_only"
...
def build_durable_executor_authoring_command_contract(
    requested: bool,
    ...
    unsafe_authoring_command_record_count = sum(
        int(_attempted(command_record.get(key))) for key in UNSAFE_RECORD_ACTION_KEYS
    )
    ...
    if requested:
        if not authoring_enable_inputs_satisfied:
            missing.append("section_112_authoring_enable_inputs_satisfied")
        if not authoring_enable_record_valid:
            missing.append("section_112_authoring_enable_record_valid")
        if not allowed_authoring_enable_observed:
            missing.append("section_112_allowed_authoring_enable_observed")
        if not no_forbidden_authoring_enable_claims:
            missing.append("section_112_no_forbidden_authoring_enable_claims")
        if not target_package_allowlist_reconfirmed:
            missing.append("section_112_target_package_allowlist_reconfirmed")
        if not overwrite_rename_decision_reconfirmed:
            missing.append("section_112_overwrite_rename_decision_reconfirmed")
        if not rollback_readiness_reconfirmed:
            missing.append("section_112_rollback_readiness_reconfirmed")
        if not ownership_marker_reconfirmed:
            missing.append("section_112_ownership_marker_reconfirmed")
        ...
        if not command_scope_matches:
            missing.append("durable_executor_authoring_command_only_scope")
        ...
        if not command_status_passed:
            missing.append("durable_authoring_command_status_passed")
    ...
    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_command",
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_SCHEMA,
        "requested": requested,
        "authoring_command_contract_defined": authoring_command_contract_defined,
        "required_authoring_command_record_schema": (
            DURABLE_EXECUTOR_AUTHORING_COMMAND_RECORD_SCHEMA if requested else ""
        ),
...
def summarize_durable_executor_authoring_commands(
    ...
    rejected_count = sum(
        ...
        if contract.get("authoring_command_record_rejected")
    )
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_command_count": len(requested),
