...
after-decision record is valid. It admits only no-op and validation result
evidence; it does not accept completed, write, dirty-package, save,
delete/rename, cleanup, code, or live command results.
"""
...
DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_RESULT_AFTER_APPLICATION_SCHEMA = (
    "section_135_durable_executor_authoring_command_completion_result_after_application_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_RESULT_AFTER_APPLICATION_RECORD_SCHEMA = (
    "section_135_durable_executor_authoring_command_completion_result_after_application_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_RESULT_AFTER_APPLICATION_SUMMARY_SCHEMA = (
    "section_135_durable_executor_authoring_command_completion_result_after_application_summary_v1"
)
EXPECTED_RESULT_SCOPE = (
    "durable_executor_authoring_command_completion_result_after_application_only"
)
...
def build_durable_executor_authoring_command_completion_result_after_application_contract(
    requested: bool,
    application_after_decision_summary: Dict[str, Any],
    ...
    application_contract_ready = bool(
        requested
        and application_after_decision_summary.get("status") == "passed"
        and application_after_decision_summary.get("application_contract_defined_count")
        == 1
        and application_after_decision_summary.get("application_record_rejected_count")
        == 0
        and application_after_decision_summary.get("unsafe_application_record_count")
        == 0
        and application_after_decision_summary.get(
            "reported_forbidden_evidence_command_count"
        )
        == 0
        and all(
            application_after_decision_summary.get(key) == 0
            for key in PREVIOUS_ZERO_COUNT_KEYS
        )
    )
    application_inputs_satisfied = bool(
        application_after_decision_summary.get("application_inputs_satisfied_count")
        == 1
    )
    application_record_valid = bool(
        application_after_decision_summary.get("application_record_valid_count") == 1
    )
    ...
    if requested:
        if not application_inputs_satisfied:
            missing.append("section_134_application_inputs_satisfied")
        if not application_record_valid:
            missing.append("section_134_application_record_valid")
    ...
    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_command_completion_result_after_application",
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_RESULT_AFTER_APPLICATION_SCHEMA,
        "requested": requested,
        ...
        "reported_allowed_evidence_command_count": (
            application_after_decision_summary.get(
                "reported_allowed_evidence_command_count",
                0,
            )
        ),
...
def summarize_durable_executor_authoring_command_completion_results_after_application(
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_COMMAND_COMPLETION_RESULT_AFTER_APPLICATION_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_command_completion_result_after_application_count": len(
            requested
        ),
