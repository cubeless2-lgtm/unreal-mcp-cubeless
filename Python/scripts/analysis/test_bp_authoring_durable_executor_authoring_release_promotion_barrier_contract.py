#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_release_promotion_barrier_contract.py."""
...
def build_current_release_decision_summary() -> dict:
    return {
        "schema": decision.DURABLE_EXECUTOR_AUTHORING_RELEASE_DECISION_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_release_promotion_barrier_record(**overrides: object) -> dict:
    record = {
        "schema": barrier.DURABLE_EXECUTOR_AUTHORING_RELEASE_PROMOTION_BARRIER_RECORD_SCHEMA,
...
def main() -> int:
    current_summary = build_current_release_decision_summary()
    contract = barrier.build_durable_executor_authoring_release_promotion_barrier_contract(
        True,
        current_summary,
    )
    assert (
        contract["schema"]
        == barrier.DURABLE_EXECUTOR_AUTHORING_RELEASE_PROMOTION_BARRIER_SCHEMA
    )
    ...
    assert "section_124_release_decision_inputs_satisfied" in contract[
        "missing_release_promotion_barrier_prerequisites"
    ]
    assert "section_124_release_decision_record_valid" in contract[
        "missing_release_promotion_barrier_prerequisites"
    ]
    assert "section_124_allowed_release_decision_observed" in contract[
        "missing_release_promotion_barrier_prerequisites"
    ]
    assert "section_124_no_forbidden_release_decision_claims" in contract[
        "missing_release_promotion_barrier_prerequisites"
    ]
    ...
    summary = barrier.summarize_durable_executor_authoring_release_promotion_barriers(
        [contract]
    )
    ...
    future_contract = barrier.build_durable_executor_authoring_release_promotion_barrier_contract(
        True,
        future_summary,
    ...
    unsafe_contract = barrier.build_durable_executor_authoring_release_promotion_barrier_contract(
        True,
        future_summary,
    ...
    unsafe_summary = (
        barrier.summarize_durable_executor_authoring_release_promotion_barriers(
            [unsafe_contract]
        )
    )
    ...
    activation_contract = (
        barrier.build_durable_executor_authoring_release_promotion_barrier_contract(
            True,
            future_summary,
            build_release_promotion_barrier_record(
    ...
    print(
        "BP authoring durable executor authoring release promotion barrier contract smoke passed"
    )
    return 0
