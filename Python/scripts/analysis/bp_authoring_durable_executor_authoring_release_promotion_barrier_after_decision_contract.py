...
durable authoring release decision-after-review record is valid. It does not
...
DURABLE_EXECUTOR_AUTHORING_RELEASE_PROMOTION_BARRIER_AFTER_DECISION_SCHEMA = (
    "section_141_durable_executor_authoring_release_promotion_barrier_after_decision_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_RELEASE_PROMOTION_BARRIER_AFTER_DECISION_RECORD_SCHEMA = (
    "section_141_durable_executor_authoring_release_promotion_barrier_after_decision_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_RELEASE_PROMOTION_BARRIER_AFTER_DECISION_SUMMARY_SCHEMA = (
    "section_141_durable_executor_authoring_release_promotion_barrier_after_decision_summary_v1"
)
EXPECTED_RELEASE_PROMOTION_BARRIER_SCOPE = (
    "durable_executor_authoring_release_promotion_barrier_after_decision_only"
)
...
def build_durable_executor_authoring_release_promotion_barrier_after_decision_contract(
    requested: bool,
    release_decision_after_review_summary: Dict[str, Any],
    ...
    release_decision_contract_ready = bool(
        requested
        and release_decision_after_review_summary.get("status") == "passed"
        and release_decision_after_review_summary.get(
        ...
        == 0
        and all(
            release_decision_after_review_summary.get(key) == 0
            for key in PREVIOUS_ZERO_COUNT_KEYS
        )
    )
    release_decision_inputs_satisfied = bool(
        release_decision_after_review_summary.get(
            "release_decision_inputs_satisfied_count"
        )
        == 1
    )
    release_decision_record_valid = bool(
        release_decision_after_review_summary.get("release_decision_record_valid_count")
        == 1
    )
    ...
    if requested:
        if not release_decision_inputs_satisfied:
            missing.append("section_140_release_decision_inputs_satisfied")
        if not release_decision_record_valid:
            missing.append("section_140_release_decision_record_valid")
        if not allowed_release_decision_observed:
            missing.append("section_140_allowed_release_decision_observed")
        if not no_forbidden_release_decision_claims:
            missing.append("section_140_no_forbidden_release_decision_claims")
        ...
        if not release_promotion_barrier_scope_matches:
            missing.append(
                "durable_executor_authoring_release_promotion_barrier_after_decision_only_scope"
            )
    ...
    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_release_promotion_barrier_after_decision",
        "schema": DURABLE_EXECUTOR_AUTHORING_RELEASE_PROMOTION_BARRIER_AFTER_DECISION_SCHEMA,
        "requested": requested,
        "release_promotion_barrier_contract_defined": (
            release_promotion_barrier_contract_defined
        ),
...
def summarize_durable_executor_authoring_release_promotion_barriers_after_decision(
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_RELEASE_PROMOTION_BARRIER_AFTER_DECISION_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_release_promotion_barrier_after_decision_count": len(
            requested
        ),
