#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_enable_after_open_contract.py."""
...
def build_current_open_summary() -> dict:
    return {
        "schema": executor_open.DURABLE_EXECUTOR_AUTHORING_OPEN_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_enable_record(**overrides: object) -> dict:
    record = {
        "schema": enable.DURABLE_EXECUTOR_AUTHORING_ENABLE_AFTER_OPEN_RECORD_SCHEMA,
...
def main() -> int:
    ...
    contract = enable.build_durable_executor_authoring_enable_after_open_contract(
        True,
        current_summary,
    )
    assert (
        contract["schema"]
        == enable.DURABLE_EXECUTOR_AUTHORING_ENABLE_AFTER_OPEN_SCHEMA
    )
    ...
    assert "section_127_open_inputs_satisfied" in contract[
        "missing_authoring_enable_prerequisites"
    ]
    assert "section_127_open_record_valid" in contract[
        "missing_authoring_enable_prerequisites"
    ]
    assert "section_127_allowed_open_observed" in contract[
        "missing_authoring_enable_prerequisites"
    ]
    assert "section_127_no_forbidden_open_claims" in contract[
        "missing_authoring_enable_prerequisites"
    ]
    ...
    summary = enable.summarize_durable_executor_authoring_enable_after_open(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_enable_after_open_count"] == 1
    ...
    future_contract = (
        enable.build_durable_executor_authoring_enable_after_open_contract(
            True,
            future_summary,
            build_enable_record(),
        )
    ...
    unsafe_summary = enable.summarize_durable_executor_authoring_enable_after_open(
        [unsafe_contract]
    )
    ...
    print(
        "BP authoring durable executor authoring enable after open contract smoke passed"
    )
    return 0
