...
This contract validates a future code-patch-release-decision-only record after
the code patch release review record is valid. It does not apply code patches,
...
DURABLE_EXECUTOR_CODE_PATCH_RELEASE_DECISION_SCHEMA = (
    "section_108_durable_executor_code_patch_release_decision_contract_v1"
)
DURABLE_EXECUTOR_CODE_PATCH_RELEASE_DECISION_RECORD_SCHEMA = (
    "section_108_durable_executor_code_patch_release_decision_record_v1"
)
DURABLE_EXECUTOR_CODE_PATCH_RELEASE_DECISION_SUMMARY_SCHEMA = (
    "section_108_durable_executor_code_patch_release_decision_summary_v1"
)
EXPECTED_CODE_PATCH_RELEASE_DECISION_SCOPE = (
    "durable_executor_code_patch_release_decision_only"
)


OUTPUT_ACTION_KEYS = (
    "durable_executor_code_patch_release_decision_started",
    "durable_executor_code_patch_release_decision_accepted",
...
PREVIOUS_ZERO_COUNT_KEYS = (
    ...
    "durable_executor_code_patch_release_review_accepted_count",
...
ALLOWED_RELEASE_DECISION_COUNT_KEYS = (
    ...
    "reported_no_asset_change_release_decision_count",
    "reported_no_live_probe_release_decision_count",
)
...
def build_durable_executor_code_patch_release_decision_contract(
    requested: bool,
    code_patch_release_review_summary: Dict[str, Any],
    decision_record: Dict[str, Any] | None = None,
) -> Dict[str, Any]:
    decision_record = decision_record or {}
    decision_record_present = bool(decision_record)
    code_patch_release_review_contract_ready = bool(
        requested
        and code_patch_release_review_summary.get("status") == "passed"
        and code_patch_release_review_summary.get(
            "code_patch_release_review_contract_defined_count"
        )
        == 1
        and code_patch_release_review_summary.get(
            "code_patch_release_review_record_rejected_count"
        )
        == 0
        and code_patch_release_review_summary.get(
        ...
        == 0
        and all(
            code_patch_release_review_summary.get(key) == 0
            for key in PREVIOUS_ZERO_COUNT_KEYS
        )
    )
    code_patch_release_review_inputs_satisfied = bool(
        code_patch_release_review_summary.get(
            "code_patch_release_review_inputs_satisfied_count"
        )
        == 1
    )
    code_patch_release_review_record_valid = bool(
        code_patch_release_review_summary.get(
            "code_patch_release_review_record_valid_count"
        )
        == 1
    )
    allowed_code_patch_release_review_observed = bool(
        code_patch_release_review_summary.get(
            "allowed_code_patch_release_review_observed_count"
        )
        == 1
    )
    no_forbidden_code_patch_release_review_claims = bool(
        code_patch_release_review_summary.get(
            "no_forbidden_code_patch_release_review_claims_count"
        )
        == 1
    )
    code_patch_release_decision_inputs_satisfied = bool(
        code_patch_release_review_inputs_satisfied
        and code_patch_release_review_record_valid
    ...
    code_patch_release_decision_scope_matches = bool(
        decision_record_present
        and decision_record.get("code_patch_release_decision_scope")
        == EXPECTED_CODE_PATCH_RELEASE_DECISION_SCOPE
    )
    explicit_code_patch_release_decision_authorized = bool(
        decision_record_present
        and decision_record.get(
            "explicit_durable_executor_code_patch_release_decision_authorized"
        )
        is True
    )
    ...
    reported_allowed_code_patch_release_decision_count = sum(allowed_counts.values())
    reported_forbidden_code_patch_release_decision_count = sum(forbidden_counts.values())
    allowed_code_patch_release_decision_observed = bool(
        decision_record_present
        and reported_allowed_code_patch_release_decision_count > 0
    )
    no_forbidden_code_patch_release_decision_claims = bool(
        decision_record_present
        and reported_forbidden_code_patch_release_decision_count == 0
    )
    unsafe_code_patch_release_decision_record_count = (
        sum(int(_attempted(decision_record.get(key))) for key in UNSAFE_RECORD_ACTION_KEYS)
        + reported_forbidden_code_patch_release_decision_count
    )
    code_patch_release_decision_contract_defined = bool(
        requested and code_patch_release_review_contract_ready
    )
    code_patch_release_decision_record_valid = bool(
        code_patch_release_decision_contract_defined
        and code_patch_release_decision_inputs_satisfied
        and record_schema_matches
        and code_patch_release_decision_scope_matches
        and explicit_code_patch_release_decision_authorized
        ...
        and unsafe_code_patch_release_decision_record_count == 0
    )
    missing = []
    if requested:
        if not code_patch_release_review_inputs_satisfied:
            missing.append("section_107_code_patch_release_review_inputs_satisfied")
        if not code_patch_release_review_record_valid:
            missing.append("section_107_code_patch_release_review_record_valid")
        if not allowed_code_patch_release_review_observed:
            missing.append("section_107_allowed_code_patch_release_review_observed")
        if not no_forbidden_code_patch_release_review_claims:
            missing.append(
                "section_107_no_forbidden_code_patch_release_review_claims"
            )
        if not decision_record_present:
            missing.append("durable_executor_code_patch_release_decision_record_present")
        if not record_schema_matches:
            missing.append("durable_executor_code_patch_release_decision_record_schema")
        if not code_patch_release_decision_scope_matches:
            missing.append("durable_executor_code_patch_release_decision_only_scope")
        if not explicit_code_patch_release_decision_authorized:
            missing.append(
                "explicit_durable_executor_code_patch_release_decision_authorization"
            )
        if not release_decision_status_passed:
            missing.append("durable_executor_code_patch_release_decision_status_passed")
        ...
        if not allowed_code_patch_release_decision_observed:
            missing.append("allowed_durable_executor_code_patch_release_decision_observed")
        if not no_forbidden_code_patch_release_decision_claims:
            missing.append(
                "no_forbidden_durable_executor_code_patch_release_decision_claims"
            )
        missing.append("separate_durable_executor_release_promotion_barrier_contract")
    code_patch_release_decision_record_rejected = bool(
        decision_record_present and not code_patch_release_decision_record_valid
    )
    contract: Dict[str, Any] = {
        "id": "durable_executor_code_patch_release_decision",
        "schema": DURABLE_EXECUTOR_CODE_PATCH_RELEASE_DECISION_SCHEMA,
        "requested": requested,
        "code_patch_release_decision_contract_defined": (
            code_patch_release_decision_contract_defined
        ),
        "required_code_patch_release_decision_record_schema": (
            DURABLE_EXECUTOR_CODE_PATCH_RELEASE_DECISION_RECORD_SCHEMA
            if requested
            else ""
        ),
        "expected_code_patch_release_decision_scope": (
            EXPECTED_CODE_PATCH_RELEASE_DECISION_SCOPE if requested else ""
        ),
        "code_patch_release_review_contract_ready": (
            code_patch_release_review_contract_ready
        ),
        "code_patch_release_review_inputs_satisfied": (
            code_patch_release_review_inputs_satisfied
        ...
        "code_patch_release_decision_record_present": decision_record_present,
        "record_schema_matches": record_schema_matches,
        "code_patch_release_decision_scope_matches": (
            code_patch_release_decision_scope_matches
        ),
        "explicit_code_patch_release_decision_authorized": (
            explicit_code_patch_release_decision_authorized
        ),
        ...
        "reported_allowed_code_patch_release_decision_count": (
            reported_allowed_code_patch_release_decision_count
        ),
        "reported_forbidden_code_patch_release_decision_count": (
            reported_forbidden_code_patch_release_decision_count
        ...
        "code_patch_release_decision_record_rejected": (
            code_patch_release_decision_record_rejected
        ),
        "unsafe_code_patch_release_decision_record_count": (
            unsafe_code_patch_release_decision_record_count
        ),
        "missing_code_patch_release_decision_prerequisites": missing,
        "missing_code_patch_release_decision_prerequisite_count": len(missing),
    }
...
def summarize_durable_executor_code_patch_release_decisions(
    ...
    rejected_count = sum(
        ...
        if contract.get("code_patch_release_decision_record_rejected")
    )
    unsafe_count = _sum_count(
        requested, "unsafe_code_patch_release_decision_record_count"
    )
    forbidden_release_decision_count = _sum_count(
        requested, "reported_forbidden_code_patch_release_decision_count"
    )
    status = "not_requested"
    if requested:
        status = (
            "passed"
            if _sum_truthy(requested, "code_patch_release_decision_contract_defined")
            == len(requested)
            and rejected_count == 0
            and unsafe_count == 0
            and forbidden_release_decision_count == 0
            and all(_sum_truthy(requested, key) == 0 for key in OUTPUT_ACTION_KEYS)
            else "failed"
        )
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_CODE_PATCH_RELEASE_DECISION_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_code_patch_release_decision_count": len(
            requested
        ),
        "code_patch_release_decision_contract_defined_count": _sum_truthy(
            requested, "code_patch_release_decision_contract_defined"
        ),
        "code_patch_release_review_contract_ready_count": _sum_truthy(
            requested, "code_patch_release_review_contract_ready"
        ),
        ...
        "code_patch_release_decision_record_present_count": _sum_truthy(
            requested, "code_patch_release_decision_record_present"
        ),
        "record_schema_matches_count": _sum_truthy(requested, "record_schema_matches"),
        "code_patch_release_decision_scope_matches_count": _sum_truthy(
            requested, "code_patch_release_decision_scope_matches"
        ),
        "explicit_code_patch_release_decision_authorized_count": _sum_truthy(
            requested, "explicit_code_patch_release_decision_authorized"
        ),
        ...
        "code_patch_release_decision_record_rejected_count": rejected_count,
        "unsafe_code_patch_release_decision_record_count": unsafe_count,
        "missing_code_patch_release_decision_prerequisite_count": _sum_count(
            requested, "missing_code_patch_release_decision_prerequisite_count"
        ),
        "reported_allowed_code_patch_release_decision_count": _sum_count(
            requested, "reported_allowed_code_patch_release_decision_count"
        ),
        "reported_forbidden_code_patch_release_decision_count": (
            forbidden_release_decision_count
        ),
    }
