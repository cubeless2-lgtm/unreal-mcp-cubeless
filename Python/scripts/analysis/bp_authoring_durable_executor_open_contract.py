...
This contract validates a future durable-executor-open-only record after the
activation readiness record is valid. It does not activate or open the durable
executor, modify code or assets, probe live bridges, enable durable authoring,
...
DURABLE_EXECUTOR_OPEN_SCHEMA = "section_111_durable_executor_open_contract_v1"
DURABLE_EXECUTOR_OPEN_RECORD_SCHEMA = "section_111_durable_executor_open_record_v1"
DURABLE_EXECUTOR_OPEN_SUMMARY_SCHEMA = "section_111_durable_executor_open_summary_v1"
EXPECTED_OPEN_SCOPE = "durable_executor_open_only"
...
ALLOWED_OPEN_COUNT_KEYS = (
    ...
    "reported_no_asset_change_open_count",
    "reported_no_live_probe_open_count",
)
...
def build_durable_executor_open_contract(
    requested: bool,
    activation_readiness_summary: Dict[str, Any],
    ...
    activation_readiness_contract_ready = bool(
        requested
        and activation_readiness_summary.get("status") == "passed"
        and activation_readiness_summary.get("activation_readiness_contract_defined_count")
        == 1
        and activation_readiness_summary.get(
            "activation_readiness_record_rejected_count"
        )
        == 0
        and activation_readiness_summary.get(
        ...
        == 0
        and all(
            activation_readiness_summary.get(key) == 0
            for key in PREVIOUS_ZERO_COUNT_KEYS
        )
    )
    activation_readiness_inputs_satisfied = bool(
        activation_readiness_summary.get(
            "activation_readiness_inputs_satisfied_count"
        )
        == 1
    )
    activation_readiness_record_valid = bool(
        activation_readiness_summary.get("activation_readiness_record_valid_count")
        == 1
    )
    allowed_activation_readiness_observed = bool(
        activation_readiness_summary.get(
            "allowed_activation_readiness_observed_count"
        )
        == 1
    )
    no_forbidden_activation_readiness_claims = bool(
        activation_readiness_summary.get(
            "no_forbidden_activation_readiness_claims_count"
        )
        == 1
    )
    ...
    record_schema_matches = bool(
        open_record_present
        and open_record.get("schema") == DURABLE_EXECUTOR_OPEN_RECORD_SCHEMA
    )
    ...
    forbidden_counts = {
        key: _count(open_record.get(key))
        for key in FORBIDDEN_OPEN_COUNT_KEYS
    }
    ...
    allowed_open_observed = bool(
        open_record_present and reported_allowed_open_count > 0
    )
    no_forbidden_open_claims = bool(
        open_record_present and reported_forbidden_open_count == 0
    )
    unsafe_open_record_count = (
        sum(int(_attempted(open_record.get(key))) for key in UNSAFE_RECORD_ACTION_KEYS)
    ...
    if requested:
        if not activation_readiness_inputs_satisfied:
            missing.append("section_110_activation_readiness_inputs_satisfied")
        if not activation_readiness_record_valid:
            missing.append("section_110_activation_readiness_record_valid")
        if not allowed_activation_readiness_observed:
            missing.append("section_110_allowed_activation_readiness_observed")
        if not no_forbidden_activation_readiness_claims:
            missing.append("section_110_no_forbidden_activation_readiness_claims")
        ...
        if not allowed_open_observed:
            missing.append("allowed_durable_executor_open_observed")
        if not no_forbidden_open_claims:
            missing.append("no_forbidden_durable_executor_open_claims")
    ...
    contract: Dict[str, Any] = {
        "id": "durable_executor_open",
        "schema": DURABLE_EXECUTOR_OPEN_SCHEMA,
        "requested": requested,
        "open_contract_defined": open_contract_defined,
        "required_open_record_schema": (
            DURABLE_EXECUTOR_OPEN_RECORD_SCHEMA if requested else ""
        ),
...
def summarize_durable_executor_opens(
    contracts: Sequence[Dict[str, Any]]
) -> Dict[str, Any]:
    requested = [contract for contract in contracts if contract.get("requested")]
    rejected_count = sum(
        1 for contract in requested if contract.get("open_record_rejected")
    )
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_OPEN_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_open_count": len(requested),
        ...
        "open_record_present_count": _sum_truthy(
            requested, "open_record_present"
        ),
    ...
    summary.update(
        {key: _sum_count(requested, key) for key in ALLOWED_OPEN_COUNT_KEYS}
    )
    summary.update(
        {key: _sum_count(requested, key) for key in FORBIDDEN_OPEN_COUNT_KEYS}
    )
    return summary
