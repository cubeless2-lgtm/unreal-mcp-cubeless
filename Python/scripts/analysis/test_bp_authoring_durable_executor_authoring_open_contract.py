#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_open_contract.py."""
...
import bp_authoring_durable_executor_authoring_open_contract as executor_open  # noqa: E402


def build_current_activation_readiness_summary() -> dict:
    return {
        "schema": readiness.DURABLE_EXECUTOR_AUTHORING_ACTIVATION_READINESS_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_open_record(**overrides: object) -> dict:
    record = {
        "schema": executor_open.DURABLE_EXECUTOR_AUTHORING_OPEN_RECORD_SCHEMA,
...
def main() -> int:
    ...
    contract = executor_open.build_durable_executor_authoring_open_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == executor_open.DURABLE_EXECUTOR_AUTHORING_OPEN_SCHEMA
    ...
    assert "section_126_activation_readiness_inputs_satisfied" in contract[
        "missing_open_prerequisites"
    ]
    assert "section_126_activation_readiness_record_valid" in contract[
        "missing_open_prerequisites"
    ]
    assert "section_126_allowed_activation_readiness_observed" in contract[
        "missing_open_prerequisites"
    ]
    assert "section_126_no_forbidden_activation_readiness_claims" in contract[
        "missing_open_prerequisites"
    ]
    ...
    summary = executor_open.summarize_durable_executor_authoring_opens([contract])
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_open_count"] == 1
    ...
    future_contract = executor_open.build_durable_executor_authoring_open_contract(
        True,
        future_summary,
        build_open_record(),
    )
    ...
    unsafe_contract = executor_open.build_durable_executor_authoring_open_contract(
        True,
        future_summary,
    ...
    unsafe_summary = executor_open.summarize_durable_executor_authoring_opens(
        [unsafe_contract]
    )
    ...
    forbidden_contract = executor_open.build_durable_executor_authoring_open_contract(
        True,
        future_summary,
    ...
    print("BP authoring durable executor authoring open contract smoke passed")
    return 0
