#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_command_execution_evidence_contract.py."""
...
import bp_authoring_durable_executor_authoring_command_execution_contract as execution  # noqa: E402
...
def build_current_execution_summary() -> dict:
    return {
        "schema": execution.DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_evidence_record(**overrides: object) -> dict:
    record = {
        "schema": (
            evidence.DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_EVIDENCE_RECORD_SCHEMA
        ),
...
def main() -> int:
    ...
    contract = evidence.build_durable_executor_authoring_command_execution_evidence_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == evidence.DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_EVIDENCE_SCHEMA
    ...
    assert "section_115_execution_inputs_satisfied" in contract[
        "missing_evidence_prerequisites"
    ]
    assert "section_115_execution_record_valid" in contract[
        "missing_evidence_prerequisites"
    ]
    assert "section_115_allowed_execution_observed" in contract[
        "missing_evidence_prerequisites"
    ]
    assert "section_115_no_forbidden_execution_claims" in contract[
        "missing_evidence_prerequisites"
    ]
    assert "durable_authoring_command_execution_evidence_record_present" in contract[
        "missing_evidence_prerequisites"
    ]
    ...
    summary = evidence.summarize_durable_executor_authoring_command_execution_evidence(
        [contract]
    )
    assert summary["status"] == "passed"
    assert (
        summary["durable_requested_executor_authoring_command_execution_evidence_count"]
        == 1
    )
    ...
    admitted_contract = (
        evidence.build_durable_executor_authoring_command_execution_evidence_contract(
            True,
            future_summary,
    ...
    unsafe_summary = evidence.summarize_durable_executor_authoring_command_execution_evidence(
        [unsafe_contract]
    )
    ...
    assert unsafe_summary["durable_authoring_command_completion_decision_started_count"] == 0
    ...
    action_claim = (
        evidence.build_durable_executor_authoring_command_execution_evidence_contract(
            True,
            future_summary,
            build_evidence_record(asset_write_performed=True),
        )
    )
    ...
    print(
        "BP authoring durable executor authoring command execution evidence contract smoke passed"
    )
    return 0
