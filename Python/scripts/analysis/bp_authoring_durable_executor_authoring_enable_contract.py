...
This contract validates a future durable-executor-authoring-enable-only record
after the durable executor open record is valid. It does not open the executor,
enable durable authoring, modify code or assets, probe live bridges, save,
...
DURABLE_EXECUTOR_AUTHORING_ENABLE_SCHEMA = (
    "section_112_durable_executor_authoring_enable_contract_v1"
)
DURABLE_EXECUTOR_AUTHORING_ENABLE_RECORD_SCHEMA = (
    "section_112_durable_executor_authoring_enable_record_v1"
)
DURABLE_EXECUTOR_AUTHORING_ENABLE_SUMMARY_SCHEMA = (
    "section_112_durable_executor_authoring_enable_summary_v1"
)
EXPECTED_AUTHORING_ENABLE_SCOPE = "durable_executor_authoring_enable_only"
...
def build_durable_executor_authoring_enable_contract(
    requested: bool,
    ...
    executor_open_contract_ready = bool(
        ...
        and open_summary.get("reported_forbidden_open_count") == 0
        and all(open_summary.get(key) == 0 for key in PREVIOUS_ZERO_COUNT_KEYS)
    )
    ...
    allowed_open_observed = bool(open_summary.get("allowed_open_observed_count") == 1)
    no_forbidden_open_claims = bool(
        open_summary.get("no_forbidden_open_claims_count") == 1
    )
    ...
    unsafe_authoring_enable_record_count = (
        sum(int(_attempted(enable_record.get(key))) for key in UNSAFE_RECORD_ACTION_KEYS)
    ...
    if requested:
        if not open_inputs_satisfied:
            missing.append("section_111_open_inputs_satisfied")
        if not open_record_valid:
            missing.append("section_111_open_record_valid")
        if not allowed_open_observed:
            missing.append("section_111_allowed_open_observed")
        if not no_forbidden_open_claims:
            missing.append("section_111_no_forbidden_open_claims")
        ...
        if not enable_scope_matches:
            missing.append("durable_executor_authoring_enable_only_scope")
        ...
        if not authoring_enable_status_passed:
            missing.append("durable_authoring_enable_status_passed")
        ...
        if not allowed_authoring_enable_observed:
            missing.append("allowed_durable_authoring_enable_observed")
        if not no_forbidden_authoring_enable_claims:
            missing.append("no_forbidden_durable_authoring_enable_claims")
    ...
    contract: Dict[str, Any] = {
        "id": "durable_executor_authoring_enable",
        "schema": DURABLE_EXECUTOR_AUTHORING_ENABLE_SCHEMA,
        "requested": requested,
        "authoring_enable_contract_defined": authoring_enable_contract_defined,
        "required_authoring_enable_record_schema": (
            DURABLE_EXECUTOR_AUTHORING_ENABLE_RECORD_SCHEMA if requested else ""
        ),
...
def summarize_durable_executor_authoring_enables(
    ...
    rejected_count = sum(
        ...
        if contract.get("authoring_enable_record_rejected")
    )
    ...
    summary: Dict[str, Any] = {
        "schema": DURABLE_EXECUTOR_AUTHORING_ENABLE_SUMMARY_SCHEMA,
        "status": status,
        "durable_requested_executor_authoring_enable_count": len(requested),
