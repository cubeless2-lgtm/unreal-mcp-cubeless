#!/usr/bin/env python
"""Offline smoke tests for Section 165 command execution dry-run contract."""
...
import bp_authoring_durable_executor_authoring_command_dispatch_evidence_dry_run_contract as evidence  # noqa: E402
import bp_authoring_durable_executor_authoring_command_execution_dry_run_contract as execution  # noqa: E402


def build_section_164_summary(**overrides: object) -> dict:
    summary = {
        "schema": evidence.DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_EVIDENCE_DRY_RUN_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_execution_record(**overrides: object) -> dict:
    record = {
        "schema": execution.DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_DRY_RUN_RECORD_SCHEMA,
        "execution_scope": execution.EXPECTED_EXECUTION_SCOPE,
        "status": "passed",
        "dry_run_only": True,
        "requested_command": "create_blueprint_asset",
        "execution_operation": "execution_dry_run_only",
        ...
        "dispatch_evidence_admission_proof": {
...
def main() -> int:
    current_summary = build_section_164_summary()
    contract = execution.build_durable_executor_authoring_command_execution_dry_run_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == execution.DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_DRY_RUN_SCHEMA
    assert contract["requested"] is True
    assert contract["execution_contract_defined"] is True
    assert contract["section_164_dispatch_evidence_contract_ready"] is True
    assert contract["evidence_chain_satisfied"] is False
    assert contract["execution_dry_run_record_present"] is False
    assert contract["execution_dry_run_record_valid"] is False
    assert contract["execution_dry_run_record_rejected"] is False
    assert contract["execution_dry_run_admissible"] is False
    assert contract["missing_execution_dry_run_prerequisite_count"] == 25
    assert "section_164_dispatch_evidence_dry_run_admissible" in contract[
        "missing_execution_dry_run_prerequisites"
    ]
    assert "command_execution_dry_run_record_present" in contract[
        "missing_execution_dry_run_prerequisites"
    ]
    assert contract["durable_execution_envelope_promoted"] is False
    assert contract["durable_evidence_promoted"] is False
    ...
    summary = execution.summarize_durable_executor_authoring_command_execution_dry_runs(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_command_execution_dry_run_count"] == 1
    assert summary["execution_contract_defined_count"] == 1
    assert summary["section_164_dispatch_evidence_contract_ready_count"] == 1
    assert summary["evidence_chain_satisfied_count"] == 0
    assert summary["execution_dry_run_record_present_count"] == 0
    assert summary["execution_dry_run_record_valid_count"] == 0
    assert summary["execution_dry_run_record_rejected_count"] == 0
    assert summary["execution_dry_run_admissible_count"] == 0
    assert summary["durable_execution_envelope_promoted_count"] == 0
    assert summary["durable_evidence_promoted_count"] == 0
    ...
    blocked_with_record = execution.build_durable_executor_authoring_command_execution_dry_run_contract(
        True,
        current_summary,
        build_execution_record(),
    )
    assert blocked_with_record["evidence_chain_satisfied"] is False
    assert blocked_with_record["execution_dry_run_record_valid"] is False
    assert blocked_with_record["execution_dry_run_record_rejected"] is True
    assert blocked_with_record["execution_dry_run_admissible"] is False
    assert blocked_with_record["missing_execution_dry_run_prerequisite_count"] == 11
    assert blocked_with_record["durable_execution_envelope_promoted"] is False
    ...
    future_summary = build_section_164_summary(
    ...
    future_contract = execution.build_durable_executor_authoring_command_execution_dry_run_contract(
        True,
        future_summary,
        build_execution_record(),
    )
    assert future_contract["evidence_chain_satisfied"] is True
    assert future_contract["execution_dry_run_record_valid"] is True
    assert future_contract["execution_dry_run_record_rejected"] is False
    assert future_contract["execution_dry_run_admissible"] is True
    assert future_contract["missing_execution_dry_run_prerequisite_count"] == 0
    ...
    future_summary_result = execution.summarize_durable_executor_authoring_command_execution_dry_runs(
        [future_contract]
    )
    assert future_summary_result["status"] == "passed"
    assert future_summary_result["execution_dry_run_admissible_count"] == 1
    assert future_summary_result["durable_execution_envelope_promoted_count"] == 0
    assert future_summary_result["durable_evidence_promoted_count"] == 0
    ...
    forbidden_contract = execution.build_durable_executor_authoring_command_execution_dry_run_contract(
        True,
        future_summary,
        build_execution_record(requested_command="save_asset"),
    )
    assert forbidden_contract["requested_command_forbidden"] is True
    assert forbidden_contract["execution_dry_run_record_rejected"] is True
    assert forbidden_contract["execution_dry_run_admissible"] is False
    forbidden_summary = execution.summarize_durable_executor_authoring_command_execution_dry_runs(
        [forbidden_contract]
    )
    ...
    unsafe_contract = execution.build_durable_executor_authoring_command_execution_dry_run_contract(
        True,
        future_summary,
        build_execution_record(live_command_executed=True),
    )
    assert unsafe_contract["unsafe_execution_record_count"] == 1
    assert unsafe_contract["execution_dry_run_record_rejected"] is True
    assert unsafe_contract["execution_dry_run_admissible"] is False
    ...
    print("BP authoring durable executor authoring command execution dry-run contract smoke passed")
    return 0
