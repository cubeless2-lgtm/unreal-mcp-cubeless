#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_enable_after_open_after_activation_readiness_contract.py."""
...
def build_current_open_after_activation_readiness_summary() -> dict:
    return {
        "schema": executor_open.DURABLE_EXECUTOR_AUTHORING_OPEN_AFTER_ACTIVATION_READINESS_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_enable_record(**overrides: object) -> dict:
    record = {
        "schema": enable.DURABLE_EXECUTOR_AUTHORING_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_RECORD_SCHEMA,
...
def main() -> int:
    ...
    contract = enable.build_durable_executor_authoring_enable_after_open_after_activation_readiness_contract(
        True,
        current_summary,
    )
    assert (
        contract["schema"]
        == enable.DURABLE_EXECUTOR_AUTHORING_ENABLE_AFTER_OPEN_AFTER_ACTIVATION_READINESS_SCHEMA
    )
    ...
    assert "section_143_open_inputs_satisfied" in contract[
        "missing_authoring_enable_prerequisites"
    ]
    assert "section_143_open_record_valid" in contract[
        "missing_authoring_enable_prerequisites"
    ]
    assert "section_143_allowed_open_observed" in contract[
        "missing_authoring_enable_prerequisites"
    ]
    assert "section_143_no_forbidden_open_claims" in contract[
        "missing_authoring_enable_prerequisites"
    ]
    ...
    assert "separate_durable_authoring_command_after_enable_contract" in contract[
        "missing_authoring_enable_prerequisites"
    ]
    ...
    summary = enable.summarize_durable_executor_authoring_enable_after_open_after_activation_readiness(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_enable_after_open_after_activation_readiness_count"] == 1
    ...
    future_contract = (
        enable.build_durable_executor_authoring_enable_after_open_after_activation_readiness_contract(
            True,
            future_summary,
            build_enable_record(),
        )
    ...
    assert future_contract["missing_authoring_enable_prerequisites"] == [
        "separate_durable_authoring_command_after_enable_contract"
    ]
    ...
    unsafe_summary = enable.summarize_durable_executor_authoring_enable_after_open_after_activation_readiness(
        [unsafe_contract]
    )
    ...
    print(
        "BP authoring durable executor authoring enable-after-open-after-activation-readiness contract smoke passed"
    )
    return 0
