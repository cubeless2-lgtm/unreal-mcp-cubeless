#!/usr/bin/env python
"""Offline smoke tests for Section 164 command dispatch evidence dry-run contract."""
...
def build_section_163_summary(**overrides: object) -> dict:
    summary = {
        "schema": dispatch.DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_DRY_RUN_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_evidence_record(**overrides: object) -> dict:
    record = {
        "schema": evidence.DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_EVIDENCE_DRY_RUN_RECORD_SCHEMA,
        "evidence_scope": evidence.EXPECTED_EVIDENCE_SCOPE,
        "status": "passed",
        "dry_run_only": True,
        "requested_command": "create_blueprint_asset",
        "evidence_operation": "dispatch_evidence_dry_run_only",
...
def main() -> int:
    current_summary = build_section_163_summary()
    contract = evidence.build_durable_executor_authoring_command_dispatch_evidence_dry_run_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == evidence.DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_EVIDENCE_DRY_RUN_SCHEMA
    assert contract["requested"] is True
    assert contract["dispatch_evidence_contract_defined"] is True
    assert contract["section_163_dispatch_contract_ready"] is True
    assert contract["dispatch_chain_satisfied"] is False
    assert contract["dispatch_evidence_dry_run_record_present"] is False
    assert contract["dispatch_evidence_dry_run_record_valid"] is False
    assert contract["dispatch_evidence_dry_run_record_rejected"] is False
    assert contract["dispatch_evidence_dry_run_admissible"] is False
    assert contract["missing_dispatch_evidence_dry_run_prerequisite_count"] == 23
    assert "section_163_dispatch_dry_run_admissible" in contract[
        "missing_dispatch_evidence_dry_run_prerequisites"
    ]
    assert "command_dispatch_evidence_dry_run_record_present" in contract[
        "missing_dispatch_evidence_dry_run_prerequisites"
    ]
    ...
    summary = evidence.summarize_durable_executor_authoring_command_dispatch_evidence_dry_runs(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_command_dispatch_evidence_dry_run_count"] == 1
    assert summary["dispatch_evidence_contract_defined_count"] == 1
    assert summary["section_163_dispatch_contract_ready_count"] == 1
    assert summary["dispatch_chain_satisfied_count"] == 0
    assert summary["dispatch_evidence_dry_run_record_present_count"] == 0
    assert summary["dispatch_evidence_dry_run_record_valid_count"] == 0
    assert summary["dispatch_evidence_dry_run_record_rejected_count"] == 0
    assert summary["dispatch_evidence_dry_run_admissible_count"] == 0
    ...
    blocked_with_record = evidence.build_durable_executor_authoring_command_dispatch_evidence_dry_run_contract(
        True,
        current_summary,
        build_evidence_record(),
    )
    assert blocked_with_record["dispatch_chain_satisfied"] is False
    assert blocked_with_record["dispatch_evidence_dry_run_record_valid"] is False
    assert blocked_with_record["dispatch_evidence_dry_run_record_rejected"] is True
    assert blocked_with_record["dispatch_evidence_dry_run_admissible"] is False
    assert blocked_with_record["missing_dispatch_evidence_dry_run_prerequisite_count"] == 9
    assert blocked_with_record["durable_evidence_promoted"] is False
    ...
    future_summary = build_section_163_summary(
    ...
    future_contract = evidence.build_durable_executor_authoring_command_dispatch_evidence_dry_run_contract(
        True,
        future_summary,
        build_evidence_record(),
    )
    assert future_contract["dispatch_chain_satisfied"] is True
    assert future_contract["dispatch_evidence_dry_run_record_valid"] is True
    assert future_contract["dispatch_evidence_dry_run_record_rejected"] is False
    assert future_contract["dispatch_evidence_dry_run_admissible"] is True
    assert future_contract["missing_dispatch_evidence_dry_run_prerequisite_count"] == 0
    ...
    future_summary_result = evidence.summarize_durable_executor_authoring_command_dispatch_evidence_dry_runs(
        [future_contract]
    )
    assert future_summary_result["status"] == "passed"
    assert future_summary_result["dispatch_evidence_dry_run_admissible_count"] == 1
    ...
    forbidden_contract = evidence.build_durable_executor_authoring_command_dispatch_evidence_dry_run_contract(
        True,
        future_summary,
        build_evidence_record(requested_command="save_asset"),
    )
    assert forbidden_contract["requested_command_forbidden"] is True
    assert forbidden_contract["dispatch_evidence_dry_run_record_rejected"] is True
    assert forbidden_contract["dispatch_evidence_dry_run_admissible"] is False
    forbidden_summary = evidence.summarize_durable_executor_authoring_command_dispatch_evidence_dry_runs(
        [forbidden_contract]
    )
    ...
    unsafe_contract = evidence.build_durable_executor_authoring_command_dispatch_evidence_dry_run_contract(
        True,
        future_summary,
        build_evidence_record(live_command_executed=True),
    )
    assert unsafe_contract["unsafe_evidence_record_count"] == 1
    assert unsafe_contract["dispatch_evidence_dry_run_record_rejected"] is True
    assert unsafe_contract["dispatch_evidence_dry_run_admissible"] is False
    ...
    print("BP authoring durable executor authoring command dispatch evidence dry-run contract smoke passed")
    return 0
