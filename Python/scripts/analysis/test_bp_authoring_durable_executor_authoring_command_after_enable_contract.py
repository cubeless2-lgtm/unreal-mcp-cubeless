#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_command_after_enable_contract.py."""
...
import bp_authoring_durable_executor_authoring_enable_after_open_contract as enable  # noqa: E402


def build_current_authoring_enable_summary() -> dict:
    return {
        "schema": enable.DURABLE_EXECUTOR_AUTHORING_ENABLE_AFTER_OPEN_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_command_record(**overrides: object) -> dict:
    record = {
        "schema": command.DURABLE_EXECUTOR_AUTHORING_COMMAND_AFTER_ENABLE_RECORD_SCHEMA,
...
def main() -> int:
    ...
    contract = command.build_durable_executor_authoring_command_after_enable_contract(
        True,
        current_summary,
    )
    assert (
        contract["schema"]
        == command.DURABLE_EXECUTOR_AUTHORING_COMMAND_AFTER_ENABLE_SCHEMA
    )
    ...
    assert "section_128_authoring_enable_inputs_satisfied" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "section_128_authoring_enable_record_valid" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "section_128_allowed_authoring_enable_observed" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "section_128_no_forbidden_authoring_enable_claims" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "durable_authoring_command_after_enable_record_present" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "durable_executor_authoring_command_after_enable_only_scope" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "explicit_durable_mvp_request_reconfirmed" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "separate_durable_authoring_command_dispatch_contract" in contract[
        "missing_authoring_command_prerequisites"
    ]
    ...
    summary = command.summarize_durable_executor_authoring_commands_after_enable(
        [contract]
    )
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_command_after_enable_count"] == 1
    ...
    future_contract = command.build_durable_executor_authoring_command_after_enable_contract(
        True,
        future_summary,
    ...
    forbidden_contract = command.build_durable_executor_authoring_command_after_enable_contract(
        True,
        future_summary,
    ...
    forbidden_summary = command.summarize_durable_executor_authoring_commands_after_enable(
        [forbidden_contract]
    )
    ...
    unsafe_contract = command.build_durable_executor_authoring_command_after_enable_contract(
        True,
        future_summary,
    ...
    print(
        "BP authoring durable executor authoring command after enable contract smoke passed"
    )
    return 0
