#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_command_execution_contract.py."""
...
def build_current_dispatch_summary() -> dict:
    return {
        "schema": dispatch.DURABLE_EXECUTOR_AUTHORING_COMMAND_DISPATCH_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_execution_record(**overrides: object) -> dict:
    record = {
        "schema": execution.DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_RECORD_SCHEMA,
...
def main() -> int:
    ...
    contract = execution.build_durable_executor_authoring_command_execution_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == execution.DURABLE_EXECUTOR_AUTHORING_COMMAND_EXECUTION_SCHEMA
    ...
    assert "section_114_dispatch_inputs_satisfied" in contract[
        "missing_execution_prerequisites"
    ]
    assert "section_114_dispatch_record_valid" in contract[
        "missing_execution_prerequisites"
    ]
    assert "section_114_allowed_dispatch_observed" in contract[
        "missing_execution_prerequisites"
    ]
    assert "section_114_no_forbidden_dispatch_claims" in contract[
        "missing_execution_prerequisites"
    ]
    assert "durable_authoring_command_execution_record_present" in contract[
        "missing_execution_prerequisites"
    ]
    ...
    summary = execution.summarize_durable_executor_authoring_command_executions([contract])
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_command_execution_count"] == 1
    ...
    future_contract = execution.build_durable_executor_authoring_command_execution_contract(
        True,
        future_summary,
    ...
    unsafe_contract = execution.build_durable_executor_authoring_command_execution_contract(
        True,
        future_summary,
    ...
    unsafe_summary = execution.summarize_durable_executor_authoring_command_executions(
        [unsafe_contract]
    )
    ...
    forbidden_contract = execution.build_durable_executor_authoring_command_execution_contract(
        True,
        future_summary,
        build_execution_record(reported_live_execution_count=1),
    )
    ...
    print("BP authoring durable executor authoring command execution contract smoke passed")
    return 0
