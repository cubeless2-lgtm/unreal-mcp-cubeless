#!/usr/bin/env python
"""Offline smoke tests for bp_authoring_durable_executor_authoring_command_contract.py."""
...
def build_current_authoring_enable_summary() -> dict:
    return {
        "schema": enable.DURABLE_EXECUTOR_AUTHORING_ENABLE_SUMMARY_SCHEMA,
        "status": "passed",
...
def build_command_record(**overrides: object) -> dict:
    record = {
        "schema": command.DURABLE_EXECUTOR_AUTHORING_COMMAND_RECORD_SCHEMA,
...
def main() -> int:
    ...
    contract = command.build_durable_executor_authoring_command_contract(
        True,
        current_summary,
    )
    assert contract["schema"] == command.DURABLE_EXECUTOR_AUTHORING_COMMAND_SCHEMA
    ...
    assert "section_112_authoring_enable_inputs_satisfied" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "section_112_authoring_enable_record_valid" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "section_112_allowed_authoring_enable_observed" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "section_112_no_forbidden_authoring_enable_claims" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "durable_authoring_command_record_present" in contract[
        "missing_authoring_command_prerequisites"
    ]
    assert "durable_executor_authoring_command_only_scope" in contract[
        "missing_authoring_command_prerequisites"
    ]
    ...
    summary = command.summarize_durable_executor_authoring_commands([contract])
    assert summary["status"] == "passed"
    assert summary["durable_requested_executor_authoring_command_count"] == 1
    ...
    future_contract = command.build_durable_executor_authoring_command_contract(
        True,
        future_summary,
    ...
    forbidden_contract = command.build_durable_executor_authoring_command_contract(
        True,
        future_summary,
    ...
    forbidden_summary = command.summarize_durable_executor_authoring_commands(
        [forbidden_contract]
    )
    ...
    unsafe_contract = command.build_durable_executor_authoring_command_contract(
        True,
        future_summary,
    ...
    print("BP authoring durable executor authoring command contract smoke passed")
    return 0
